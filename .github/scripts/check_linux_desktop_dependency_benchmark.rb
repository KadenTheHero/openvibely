...
recovery_sample_ids = recovery_samples.map { |sample| sample["benchmark_sample_id"] }.uniq
errors << "cold/partial recovery evidence must contain 3 distinct sample IDs; found #{recovery_sample_ids.length}" if recovery_sample_ids.length < 3
recovery_samples.each do |sample|
  unless sample["install_succeeded"] == true && sample["apt_update_count"].to_i.positive? && sample["metadata_validation"].to_s.include?("updated")
