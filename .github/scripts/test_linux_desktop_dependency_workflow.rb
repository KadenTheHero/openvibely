...
expect.call(key_script.include?("BENCHMARK_SCENARIO") && key_script.include?("benchmark-cold") && key_script.include?("BENCHMARK_SAMPLE_ID") && key_script.include?("sample_hash"), "cold benchmark runs must use a unique sample cache identity")
...
job_start_step = steps.find { |step| step["name"] == "Record Linux test job start" }
checkout_step = steps.find { |step| step["name"] == "Check out repo" }
expect.call(!job_start_step.nil? && job_start_step.fetch("run", "").include?("LINUX_TEST_JOB_STARTED_AT_MS"), "complete-job timing must begin before checkout")
expect.call(!job_start_step.nil? && !checkout_step.nil? && steps.index(job_start_step) < steps.index(checkout_step), "complete-job timing must begin before checkout")
