...
func TestAutomationYAMLBuilderUsesConsistentLayout(t *testing.T) {
	...
	for source, id := range map[string]string{"blank": "", "template": "", "edit": "automation-edit"} {
		...
		if !strings.Contains(body, `class="group relative w-10 shrink-0 overflow-hidden border-r border-base-300"`) || !strings.Contains(body, `class="m-0 h-full w-full select-none overflow-hidden whitespace-nowrap px-2 py-0 text-right text-xs text-base-content/45"`) || !strings.Contains(body, `data-automation-yaml-fold-controls`) || !strings.Contains(body, `w-4`) || !strings.Contains(body, `h-6 w-4`) || !strings.Contains(body, `text-lg`) || !strings.Contains(body, `whitespace-pre-wrap break-words px-3`) || !strings.Contains(body, `data-yaml-indent="' + indent + '"' + hangingIndent`) || !strings.Contains(body, `transform:translateX(-50%)`) {
...
window.addEventListener('DOMContentLoaded', function() {
  ...
  (async function() {
    ...
    var firstRailBounds = Array.from(indentRails).sort(function(left, right) { return left.getBoundingClientRect().left - right.getBoundingClientRect().left; })[0].getBoundingClientRect();
	    ...
	    if (!gutter.classList.contains('w-10') || !lineNumbers.classList.contains('w-full') || !lineNumbers.classList.contains('text-right') || !lineNumbers.classList.contains('text-xs') || !foldControls.classList.contains('w-4')) fail('YAML gutter must match the split diff viewer\'s compact w-10 number column with an overlay fold control');
	    ...
	    if (Math.abs(gutterBounds.width - 40) > 1 || Math.abs(numberBounds.width - gutterBounds.width) > 1 || numberBounds.left < gutterBounds.left - 1 || numberBounds.right > gutterBounds.right + 1 || foldControlsBounds.right > gutterBounds.right + 1) fail('YAML line numbers and fold controls must stay within the split diff viewer gutter: gutter=' + gutterBounds.left + ',' + gutterBounds.right + ' number=' + numberBounds.left + ',' + numberBounds.right + ' controls=' + foldControlsBounds.left + ',' + foldControlsBounds.right);    if (window.getComputedStyle(lineNumbers).textAlign !== 'right') fail('YAML line numbers must use the split diff viewer\'s right-aligned number-cell style');
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case "/":
			...
			_, _ = fmt.Fprintf(w, `<!doctype html><html><head><meta charset="utf-8"><style>:root{--bc:20%% 0.02 260;--er:0.68 0.15 26}body{margin:0;padding:20px}*{box-sizing:border-box}.flex{display:flex}svg[data-automation-canvas]{display:block;width:100%%;height:600px}[data-automation-yaml-gutter]{width:40px;position:relative}[data-automation-yaml-fold-controls]{position:absolute;top:0;right:0;bottom:0;width:16px}[data-automation-yaml-fold]{position:absolute;right:0;width:16px;height:24px}[data-automation-yaml-editor-viewport]{position:relative;width:320px;height:260px;overflow:hidden}[data-automation-yaml-highlight]{position:absolute;left:0;right:0;top:0;box-sizing:border-box;min-height:100%%;margin:0;padding-left:12px;font:16px/24px monospace;white-space:pre-wrap;overflow:visible;overflow-wrap:break-word}[data-automation-yaml-editor]{position:absolute;inset:0;box-sizing:border-box;width:100%%;height:100%%;margin:0;padding-left:12px;font:16px/24px monospace;white-space:pre-wrap;overflow-wrap:break-word}[data-automation-yaml-highlight-line],[data-automation-yaml-line-number]{display:block;min-height:24px}.relative{position:relative}.absolute{position:absolute}.whitespace-nowrap{white-space:nowrap}.text-left{text-align:left}.text-right{text-align:right}.px-2{padding-left:8px;padding-right:8px}.py-0{padding-top:0;padding-bottom:0}[data-automation-yaml-line-numbers]{width:100%%;margin:0;font:12px/24px monospace}</style></head><body>%s%s</body></html>`, builder.String(), runner)
		case "/browser-result":
