...
func TestAutomationYAMLBuilderUsesConsistentLayout(t *testing.T) {
	...
	for source, id := range map[string]string{"blank": "", "template": "", "edit": "automation-edit"} {
		...
		for _, want := range []string{`data-automation-yaml-highlight-line`, `whitespace-pre-wrap break-words`, `wrap="soft"`, `data-automation-yaml-line-number`} {
			...
			if !strings.Contains(body, `class="group relative w-10 shrink-0 overflow-hidden border-r border-base-300"`) || !strings.Contains(body, `class="m-0 h-full w-full select-none overflow-hidden whitespace-nowrap px-2 text-left text-xs text-base-content/45"`) || !strings.Contains(body, `data-automation-yaml-fold-controls`) || !strings.Contains(body, `w-4`) || !strings.Contains(body, `h-6 w-4`) || !strings.Contains(body, `text-lg`) || !strings.Contains(body, `whitespace-pre-wrap break-words px-3`) || !strings.Contains(body, `data-yaml-indent="' + indent + '"' + hangingIndent`) {
  ...
  (async function() {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case "/":
			...
			_, _ = fmt.Fprintf(w, `<!doctype html><html><head><meta charset="utf-8"><style>:root{--bc:20%% 0.02 260;--er:0.68 0.15 26}body{margin:0;padding:20px}.flex{display:flex}svg[data-automation-canvas]{display:block;width:100%%;height:600px}[data-automation-yaml-gutter]{width:40px;position:relative}[data-automation-yaml-fold-controls]{position:absolute;top:0;right:0;bottom:0;width:16px}[data-automation-yaml-fold]{position:absolute;right:0;width:16px;height:24px}[data-automation-yaml-editor-viewport]{position:relative;width:320px;height:260px;overflow:hidden}[data-automation-yaml-highlight]{position:absolute;left:0;right:0;top:0;box-sizing:border-box;min-height:100%%;margin:0;padding-left:12px;font:16px/24px monospace;white-space:pre-wrap;overflow:visible;overflow-wrap:break-word}[data-automation-yaml-editor]{position:absolute;inset:0;box-sizing:border-box;width:100%%;height:100%%;margin:0;padding-left:12px;font:16px/24px monospace;white-space:pre-wrap;overflow-wrap:break-word}[data-automation-yaml-highlight-line],[data-automation-yaml-line-number]{display:block;min-height:24px}.relative{position:relative}.absolute{position:absolute}.whitespace-nowrap{white-space:nowrap}[data-automation-yaml-line-numbers]{width:100%%;margin:0;font:12px/24px monospace;text-align:left}</style></head><body>%s%s</body></html>`, builder.String(), runner)
		case "/browser-result":
