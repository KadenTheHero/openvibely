...
if (!fold || window.getComputedStyle(fold).opacity === '0') fail('collapsed YAML section-fold control must remain visible in the line-number gutter');
	    ...
	    editor.dispatchEvent(new Event('input', {bubbles: true}));
	    await new Promise(function(resolve) { requestAnimationFrame(function() { requestAnimationFrame(resolve); }); });
	    if (editor.hidden || !editor.value.includes('visible_23: \"still scrollable after typing\"') || !highlight.querySelector('[data-automation-yaml-fold-summary]')) fail('typing in folded YAML must edit visible lines while preserving still-valid folded sections');
	    fold = document.querySelector('[data-automation-yaml-fold]');
	    var foldedViewport = document.querySelector('[data-automation-yaml-editor-viewport]');    if (!foldedViewport || window.getComputedStyle(foldedViewport).overflowX !== 'hidden' || editor.scrollHeight <= editor.clientHeight) fail('collapsed YAML editor must remain vertically scrollable without horizontal drift when visible lines exceed the viewport');
	    ...
	    gutter.dispatchEvent(new WheelEvent('wheel', {bubbles: true, cancelable: true, deltaY: 48}));
	    await new Promise(function(resolve) { requestAnimationFrame(resolve); });
	    if (editor.scrollTop <= 0 || editor.hidden || !foldedContent || foldedContent.getBoundingClientRect().top >= foldedContentTop - 1) fail('wheel scrolling the YAML gutter in a collapsed YAML view must move its editable visible content while retaining its folded state');    foldedViewport.click();
    if (editor.hidden || !highlight.querySelector('[data-automation-yaml-fold-summary]')) fail('clicking collapsed YAML content must not unfold it or disable editing');
		    ...
		    if (!editor.value.includes('hidden collapsed content should not widen later visible lines') || !editor.value.includes('visible_23: \"still scrollable after typing\"')) fail('expanding a folded section must restore hidden YAML content and preserve visible-line edits');
