...
func TestAutomationYAMLBuilderUsesConsistentLayout(t *testing.T) {
	...
	for source, id := range map[string]string{"blank": "", "template": "", "edit": "automation-edit"} {
		...
		if !strings.Contains(body, `data-automation-yaml-panel hidden style="display: none" class="flex min-h-[20rem] flex-1 flex-col overflow-hidden`) || !strings.Contains(body, `class="relative min-h-0 min-w-0 flex-1 overflow-hidden" data-automation-yaml-editor-viewport`) || !strings.Contains(body, `class="group relative min-w-16 shrink-0 overflow-hidden border-r border-base-300" style="box-sizing: border-box; width: max-content; min-width: 4rem; flex: 0 0 auto;"`) || !strings.Contains(body, `class="m-0 h-full w-full min-w-0 select-none overflow-hidden whitespace-nowrap pb-0 pl-2 pr-7 pt-0 text-left text-xs text-base-content/45" style="box-sizing: border-box; text-align: left !important;"`) || !strings.Contains(body, `data-automation-yaml-fold-controls`) || !strings.Contains(body, `w-4`) || !strings.Contains(body, `h-6 w-4`) || !strings.Contains(body, `text-lg`) || !strings.Contains(body, `whitespace-pre-wrap break-words px-3`) || !strings.Contains(body, `data-yaml-indent="' + indent + '"' + hangingIndent`) || !strings.Contains(body, `left:calc(' + column + 'ch + 0.65ch)`) {
  ...
  (async function() {
    ...
    var firstRailBounds = Array.from(indentRails).sort(function(left, right) { return left.getBoundingClientRect().left - right.getBoundingClientRect().left; })[0].getBoundingClientRect();
	    if (firstDotBounds.left <= firstRailBounds.right) fail('the first indentation dot must render visibly after the first vertical grouping rail');    if (!Array.from(indentRails).some(function(rail) { return rail.getBoundingClientRect().height > 24 && window.getComputedStyle(rail).backgroundColor !== 'rgba(0, 0, 0, 0)'; })) fail('YAML indentation rails must continuously span nested YAML rows with a visible color');
		    if (!gutter.classList.contains('min-w-16') || !lineNumbers.classList.contains('w-full') || !lineNumbers.classList.contains('text-left') || !lineNumbers.classList.contains('text-xs') || !lineNumbers.classList.contains('pr-7') || !foldControls.classList.contains('w-4')) fail('YAML gutter must reserve a line-number lane and a separate fold-control lane');
