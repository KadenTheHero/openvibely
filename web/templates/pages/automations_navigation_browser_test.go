...
func TestAutomationYAMLBuilderUsesConsistentLayout(t *testing.T) {
	...
	for source, id := range map[string]string{"blank": "", "template": "", "edit": "automation-edit"} {
		...
		if !strings.Contains(body, `data-automation-yaml-panel hidden style="display: none" class="flex min-h-[20rem] flex-1 flex-col overflow-auto rounded-box border border-base-300 bg-base-200/20 px-0 py-4 font-mono text-sm leading-6"`) {
...
window.addEventListener('DOMContentLoaded', function() {
  ...
  (async function() {
    ...
    var foldedViewport = document.querySelector('[data-automation-yaml-editor-viewport]');
    ...
    await new Promise(function(resolve) { requestAnimationFrame(resolve); });
    if (foldedViewport.scrollTop <= 0 || !editor.hidden) fail('scrolling a collapsed YAML view must retain its folded state');
