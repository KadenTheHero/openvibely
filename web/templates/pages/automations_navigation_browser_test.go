...
func TestAutomationBuilderEditHeaderUsesStandardSpacingAndDescriptionStyle(t *testing.T) {
	...
	for _, want := range []string{
		...
		`class="flex min-w-0 flex-shrink-0 flex-wrap items-start"`,
		`class="ml-2 flex-shrink-0 text-2xl text-base-content/30"`,
		...
		`class="input input-bordered relative -top-px ml-1 h-8 min-w-0 flex-1 px-[3px] py-0 pb-px text-xl font-bold leading-none sm:max-w-xl sm:text-2xl"`,
