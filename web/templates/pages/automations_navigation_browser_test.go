...
func TestAutomationBuilderEditHeaderUsesYAMLAuthoring(t *testing.T) {
	...
	for _, want := range []string{`data-automation-yaml-builder`, `data-automation-builder-cancel`, `data-automation-builder-save`,
		...
		`data-automation-graph-panel`, `data-automation-draft-canvas`, `min-h-[20rem] flex-1 flex-col overflow-auto rounded-box border border-base-300 bg-base-200/20 px-0 py-4 font-mono text-sm leading-6`,
	} {
		if !strings.Contains(body, want) {
...
func TestAutomationYAMLBuilderUsesConsistentLayout(t *testing.T) {
	...
	for source, id := range map[string]string{"blank": "", "template": "", "edit": "automation-edit"} {
		...
		for _, want := range []string{`data-automation-yaml-builder`, `data-automation-yaml-form`, `data-automation-yaml-editor`, `data-automation-draft-canvas`, `data-automation-graph-panel`} {
			...
			if !strings.Contains(body, `data-automation-yaml-panel hidden style="display: none" class="flex min-h-[20rem] flex-1 flex-col overflow-auto rounded-box border border-base-300 bg-base-200/20 px-0 py-4 font-mono text-sm leading-6"`) {			t.Errorf("%s YAML panel must grow to fill the builder card while remaining hidden in Graph mode", source)
		}
...
window.addEventListener('DOMContentLoaded', function() {
  ...
  (async function() {
    ...
    var firstRailBounds = Array.from(indentRails).sort(function(left, right) { return left.getBoundingClientRect().left - right.getBoundingClientRect().left; })[0].getBoundingClientRect();
	    ...
	    if (!gutter.classList.contains('w-10') || !lineNumbers.classList.contains('w-full') || !lineNumbers.classList.contains('text-left') || !lineNumbers.classList.contains('text-xs') || !foldControls.classList.contains('w-4')) fail('YAML gutter must match the split diff viewer\'s compact w-10 number column with an overlay fold control');
	    var gutterBounds = gutter.getBoundingClientRect(), editorShellBounds = editorShell.getBoundingClientRect(), numberBounds = lineNumbers.getBoundingClientRect(), foldControlsBounds = foldControls.getBoundingClientRect(), splitDiffReference = document.querySelector('[data-split-diff-gutter-reference]'), splitDiffReferenceBounds = splitDiffReference && splitDiffReference.getBoundingClientRect();
	    if (!splitDiffReferenceBounds || Math.abs(gutterBounds.width - splitDiffReferenceBounds.width) > 1) fail('YAML gutter width must match the split-diff line-number column: yaml=' + gutterBounds.width + ', split=' + (splitDiffReferenceBounds && splitDiffReferenceBounds.width));
	    if (Math.abs(gutterBounds.width - 40) > 1 || Math.abs(gutterBounds.left - editorShellBounds.left) > 1 || Math.abs(numberBounds.width - gutterBounds.width) > 1 || numberBounds.left < gutterBounds.left - 1 || numberBounds.right > gutterBounds.right + 1 || foldControlsBounds.right > gutterBounds.right + 1) fail('YAML line numbers and fold controls must stay flush with the split diff viewer gutter: shell=' + editorShellBounds.left + ',' + editorShellBounds.right + ' gutter=' + gutterBounds.left + ',' + gutterBounds.right + ' number=' + numberBounds.left + ',' + numberBounds.right + ' controls=' + foldControlsBounds.left + ',' + foldControlsBounds.right);    if (window.getComputedStyle(lineNumbers).textAlign !== 'left' || window.getComputedStyle(lineNumbers).paddingLeft !== '0px' || window.getComputedStyle(gutter).minWidth !== '40px') fail('YAML line-number gutter must be fixed-width with a flush split-diff left edge: align=' + window.getComputedStyle(lineNumbers).textAlign + ', padding=' + window.getComputedStyle(lineNumbers).paddingLeft + ', min-width=' + window.getComputedStyle(gutter).minWidth);
    var firstLineNumber = lineNumbers.querySelector('[data-automation-yaml-line-number]');
    ...
    if (firstLineNumberTextBounds.left > gutterBounds.left + 1) fail('YAML line numbers must begin at the left edge of the split-diff-width gutter: text=' + firstLineNumberTextBounds.left + ', gutter=' + gutterBounds.left);
