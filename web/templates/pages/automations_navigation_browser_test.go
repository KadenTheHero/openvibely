...
window.addEventListener('DOMContentLoaded', function() {
  ...
  (async function() {
    ...
    editor.dispatchEvent(new Event('input', {bubbles: true}));
    await new Promise(function(resolve) { requestAnimationFrame(function() { requestAnimationFrame(resolve); }); });
    var nestedRail = Array.from(highlight.querySelectorAll('[data-automation-yaml-indent-rail]')).find(function(rail) { return Number(rail.dataset.yamlRailColumn) === 0; });
    if (!nestedRail) fail('expected an indentation rail at column 0 for the nested nodes list');
    ...
    editor.dispatchEvent(new MouseEvent('mouseup', {bubbles: true, cancelable: true, clientX: railLineRect.left + 1, clientY: railLineRect.top + 2}));
    editor.dispatchEvent(new Event('click', {bubbles: true}));
    await new Promise(function(resolve) { requestAnimationFrame(resolve); });
    var activeColor = window.getComputedStyle(nestedRail).backgroundColor;
    if (activeColor === inactiveColor) fail('clicking into a YAML group must highlight its vertical indentation rail: inactive=' + inactiveColor + ' active=' + activeColor);
