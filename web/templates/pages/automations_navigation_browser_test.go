...
if (!fold || window.getComputedStyle(fold).opacity === '0') fail('collapsed YAML section-fold control must remain visible in the line-number gutter');
	    editor.value = editor.value.replace('visible_23: "still scrollable"', 'visible_23: "still scrollable after typing"');
	    editor.dispatchEvent(new Event('input', {bubbles: true}));
	    await new Promise(function(resolve) { requestAnimationFrame(function() { requestAnimationFrame(resolve); }); });
	    if (!editor.hidden || !highlight.querySelector('[data-automation-yaml-fold-summary]')) fail('typing in YAML must preserve still-valid folded sections instead of clearing all folds');
	    fold = document.querySelector('[data-automation-yaml-fold]');
	    var foldedViewport = document.querySelector('[data-automation-yaml-editor-viewport]');    if (!foldedViewport || window.getComputedStyle(foldedViewport).overflowY !== 'auto' || foldedViewport.scrollHeight <= foldedViewport.clientHeight) fail('collapsed YAML view must remain scrollable when visible lines exceed the viewport');
	    ...
	    gutter.dispatchEvent(new WheelEvent('wheel', {bubbles: true, cancelable: true, deltaY: 48}));
	    await new Promise(function(resolve) { requestAnimationFrame(resolve); });
	    if (foldedViewport.scrollTop <= 0 || !editor.hidden || !foldedContent || foldedContent.getBoundingClientRect().top >= foldedContentTop - 1) fail('wheel scrolling the YAML gutter in a collapsed YAML view must move its visible content while retaining its folded state');    foldedViewport.click();
    if (!editor.hidden) fail('clicking collapsed YAML content must not unfold it');
		    var foldedScrollTop = foldedViewport.scrollTop;
		    fold.click();
		    ...
		    if (foldedScrollTop > 0 && Math.abs(editor.scrollTop - foldedScrollTop) > 1) fail('expanding a folded section must preserve vertical position in the restored editor: folded=' + foldedScrollTop + ', editor=' + editor.scrollTop);
