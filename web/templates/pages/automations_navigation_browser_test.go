...
if (!fold || window.getComputedStyle(fold).opacity === '0') fail('collapsed YAML section-fold control must remain visible in the line-number gutter');
	    ...
	    fold = document.querySelector('[data-automation-yaml-fold]');
	    var foldedViewport = document.querySelector('[data-automation-yaml-editor-viewport]');    if (!foldedViewport || window.getComputedStyle(foldedViewport).overflowY !== 'auto' || window.getComputedStyle(foldedViewport).overflowX !== 'hidden' || foldedViewport.scrollHeight <= foldedViewport.clientHeight) fail('collapsed YAML view must remain vertically scrollable without horizontal drift when visible lines exceed the viewport');
	    if (foldedViewport.scrollWidth > foldedViewport.clientWidth + 1) fail('collapsed hidden YAML content must not widen the visible folded editor surface: scrollWidth=' + foldedViewport.scrollWidth + ', clientWidth=' + foldedViewport.clientWidth);
