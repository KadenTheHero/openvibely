...
func TestChatRenderedLinksOpenOutsideAppAndDesktopExternalBrowserBridge(t *testing.T) {
	...
	if err := layout.Base("Chat link fixture", nil, "project-links").Render(layout.WithDesktopMode(context.Background(), true), &base); err != nil {
...
func chatLinkOpeningBrowserFixture(t *testing.T, baseHTML string) string {
	...
	return `<!DOCTYPE html><html lang="en" data-openvibely-runtime="desktop" data-runtime="desktop"><head><meta charset="UTF-8"><title>Chat links</title></head><body><script>` +
...
window.addEventListener('DOMContentLoaded', function() {
  ...
  (async function() {
    ...
    function anchor(name) { return fixture.querySelector('a[data-case="' + name + '"]'); }
    ...
    ['internal', 'same-origin'].forEach(function(name) {
      ...
      if (a.hasAttribute('target') || a.hasAttribute('rel')) fail(name + ' should preserve normal internal navigation without target/rel');
    });
    var unsafe = anchor('unsafe');
    if (!unsafe) fail('missing unsafe anchor');
    if (unsafe.hasAttribute('href') || unsafe.hasAttribute('target') || unsafe.hasAttribute('rel')) fail('unsafe link was not neutralized');
    if (fixture.querySelector('pre code a')) fail('code URL was converted into a link');
