...
func TestChatContent_RestoresSmartScrollAcrossNavigationAndHistory(t *testing.T) {
	...
	if !strings.Contains(content, `id="chat-messages" class="flex-1 min-h-0 overflow-y-auto py-4 space-y-6" style="visibility: hidden;" data-transcript-hydrating="true"`) {
...
func TestChatContent_MobileComposerStaysWithinViewport(t *testing.T) {
	...
	required := []string{
		...
		`id="chat-messages" class="flex-1 min-h-0 overflow-y-auto py-4 space-y-6"`,
		`class="chat-input-shadow-gutter w-full min-w-0 max-w-full mt-2"`,
