...
- For provider-native tools with versioned schemas (OpenAI/Anthropic web tools), keep request payload types aligned with the provider’s current contract, preserve backward-compatible aliases in local allowlist/UI mappings, and cover payload shape changes with regression tests.
- For provider-managed multi-step tool loops, treat provider continuation stop reasons (for example Anthropic `pause_turn`) as resumable loop states and continue with the existing conversation context instead of finalizing early.
- For provider-native tool-result content blocks, prefer type-family pass-through (`*_tool_result`) over hardcoded per-tool allowlists so newly introduced provider tools (for example code execution) remain round-trip safe without protocol breakage.
...
- For markdown-rendered assistant/user content, escape raw HTML-like tags before markdown parsing while preserving code fences/spans; this prevents malformed model output from corrupting chat/thread DOM layout.
- When assistant responses include internal wrapper tags used for control flow (for example plan delimiters), strip only the known wrapper tags at shared display-cleaning layers and preserve inner text plus unrelated user-provided angle-bracket content.
