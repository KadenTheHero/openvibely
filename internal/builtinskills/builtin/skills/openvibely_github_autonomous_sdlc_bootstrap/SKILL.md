...
- Do not rely on prompt memory alone for duplicate prevention. Finder prompts must call `github_list_existing_automation_issues` before creating new reports, inspect similar issues with `github_get_issue` when needed, skip covered candidates, and keep searching.
