...
2. Schedule Vision Suggestions and the three finder tasks at the requested audit cadence, usually daily. Their prompts should inspect one focused area, call `list_existing_automation_notifications` before creating any notification, follow next_offset until it is zero before deciding no existing notification matches, optionally call `get_alert` for similar existing notifications, skip covered candidates, keep searching for different new findings, and create at most one new notification per run. Only call `create_notification` after the finder believes the finding is not already represented; if no new finding remains, it should report that no new notification was found. Use the role's exact notification `type`, a concise title/message, a detailed body, and structured metadata. They must not create implementation tasks or modify code.
