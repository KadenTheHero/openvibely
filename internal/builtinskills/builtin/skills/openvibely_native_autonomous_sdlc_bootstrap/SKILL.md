...
2. Schedule Vision Suggestions and the three finder tasks at the requested audit cadence, usually daily. Their prompts should inspect one focused area, call `list_existing_automation_notifications` before creating any notification, optionally call `get_alert` for similar existing notifications, skip covered candidates, keep searching for different new findings, and create at most one new notification per run. Only call `create_notification` after the finder believes the finding is not already represented; if no new finding remains, it should report that no new notification was found. Use the role's exact notification `type`, a concise title/message, a detailed body, structured metadata, and a stable `idempotency_key` in the form `<finder role>:<primary file or component>:<stable symbol or behavior>` as a retry safeguard. Reuse the exact key for a retried finding and never include run IDs, dates, timestamps, or random values. They must not create implementation tasks or modify code.
...
Every discovery prompt must choose one focused project component or workflow, vary it over time, and forbid modifying code or creating implementation tasks. Require every body to start with `## Summary`: 2-4 plain-language sentences explaining the finding, one concrete example a user would notice, and why it matters. After that summary, preserve all details useful to an implementation agent, including the inspected component, evidence and failure paths, expected versus actual behavior, risk, suggested implementation direction, acceptance criteria, file/symbol references, and regression cases. Before creating any notification, call `list_existing_automation_notifications`. If a candidate might match an existing notification, call `get_alert` for that notification and read the body. If it is covered, skip that candidate and keep searching for a different new finding. Try to create at most one new notification this run. Only call `create_notification` after you believe the finding is not already represented. If no new finding remains, report that no new notification was found. Use the role's exact type, a readable title and message, the full body, structured metadata, and a stable idempotency key in the form `<finder role>:<primary file or component>:<stable symbol or behavior>` as a retry safeguard. Reuse the exact key for a retried finding; never include run IDs, dates, timestamps, or random values.
