...
const sendMessageParams = `{"type":"object","properties":{"action":{"type":"string","enum":["send","list"],"description":"send delivers a message. list returns configured outbound targets."},"target":{"type":"string","description":"Delivery target. Format: platform, platform:target_id, platform:#target-name, or platform:target_id:thread_id. Slack/Discord authorized user IDs can be used as direct-message targets, for example slack:U123... or discord:1518288288572641398."},"message":{"type":"string","description":"Text to send."},"subject":{"type":"string","description":"Optional subject for email targets. Ignored by chat platforms."}},"additionalProperties":false}`
