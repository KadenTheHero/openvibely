...
const githubCreateIssueParams = `{"type":"object","properties":{"title":{"type":"string"},"body":{"type":"string"},"labels":{"type":"array","items":{"type":"string"},"description":"Plain GitHub labels such as suggestion, bug, approved, in-progress. Do not use an openvibely: prefix."},"assignees":{"type":"array","items":{"type":"string"}},"idempotency_key":{"type":"string","maxLength":200,"description":"Stable finding identity for Automation duplicate prevention. Reuse it when the same finding is reworded or retried; do not include a run ID, timestamp, or random value."},` + githubRepoURLProperty + `},"required":["title"],"additionalProperties":false}`
...
var registry = []ActionDef{
	...
	},
	{
		...
		AllowedModes: []models.ChatMode{models.ChatModeOrchestrate},
		Surfaces:     allSurfaces(),
		Parameters:   json.RawMessage(`{"type":"object","properties":{"project_id":{"type":"string"},"type":{"type":"string","maxLength":100},"title":{"type":"string","maxLength":200},"message":{"type":"string","maxLength":2000},"body":{"type":"string","maxLength":20000},"severity":{"type":"string","enum":["info","warning","error"]},"source":{"type":"string","maxLength":100},"metadata":{"type":"object"},"idempotency_key":{"type":"string","maxLength":200,"description":"Stable finding identity. Automation notifications require it; reuse it for reworded findings and retries, without run IDs, timestamps, or random values."}},"required":["type","title"],"additionalProperties":false}`),
	},
	{
		Name:         "claim_alert",
