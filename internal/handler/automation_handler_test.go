...
func TestAutomationTaskFollowupGitHubToolsUseHardenedRuntime(t *testing.T) {
	...
	for _, pathShape := range pathShapes {
		t.Run(pathShape.name, func(t *testing.T) {
			...
			_, handled, isError, runtimeErr := runtime.Executor(preparedCtx, "github_create_issue", json.RawMessage(`{"title":"Safe follow-up issue","assignees":["bot"],"idempotency_key":"bug:runtime:safe-follow-up-issue"}`))
			require.True(t, handled)
	...
	_, handled, isError, err = runtime.Executor(causalCtx, "github_create_issue", json.RawMessage(`{"title":"Safe follow-up issue","assignees":["bot"],"idempotency_key":"bug:runtime:safe-follow-up-issue"}`))
	require.True(t, handled)
	require.True(t, isError)
	...
	output, handled, isError, err := runtime.Executor(causalCtx, "github_create_issue", json.RawMessage(`{"title":"Safe follow-up issue","body":"body","idempotency_key":"bug:runtime:safe-follow-up-issue"}`))
	require.True(t, handled)
