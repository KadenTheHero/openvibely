...
func TestHandler_Chat_TaskCreateResultRowStyling(t *testing.T) {
	...
	body := rec.Body.String()
	assert.Contains(t, body, `--ov-link-color: #7480ff;`)
	assert.Contains(t, body, `.ov-task-result-link {`)
	assert.Contains(t, body, `color: var(--ov-link-color);`)
	assert.Contains(t, body, `text-decoration-thickness: 1px;`)
	assert.Contains(t, body, `.ov-task-result-link:visited {`)
	assert.Contains(t, body, `.ov-task-result-link:hover {`)
	assert.Contains(t, body, `.ov-task-result-link:active {`)
	assert.Contains(t, body, `.ov-task-result-link:focus-visible`)
	assert.Contains(t, body, `.chat-markdown a {`)
	assert.Contains(t, body, `.chat-markdown a:visited {`)
	assert.Contains(t, body, `.chat-markdown a:hover {`)
	assert.Contains(t, body, `.chat-markdown a:active {`)
	assert.Contains(t, body, `.chat-markdown a:focus-visible {`)
	...
	assert.NotContains(t, body, `? 'text-success underline decoration-2 underline-offset-2 hover:decoration-2 cursor-pointer font-medium'`)
	assert.NotContains(t, body, `: 'text-success hover:underline cursor-pointer font-medium';`)
...
func TestHandler_TaskThreadSend_QueueWaitDoesNotConsumeWorkerTimeout(t *testing.T) {
	...
	maxWorkers := 1
	project := &models.Project{Name: "Slow Queue Project", MaxWorkers: &maxWorkers}
	require.NoError(t, h.projectSvc.Create(ctx, project))
	h.workerSvc.SetProjectRepo(h.projectRepo)

	agent := &models.LLMConfig{
		Name:          "Short Timeout Model",
		Provider:      models.ProviderTest,
		Model:         "test-model",
		APIKey:        "test-key",
		MaxTokens:     4096,
	...
	task := &models.Task{
		ProjectID: project.ID,
		Title:     "Task for slow queued followup",
		Status:    models.StatusCompleted,
		Category:  models.CategoryCompleted,
		AgentID:   &agent.ID,
	}
	...
	select {
	case <-providerCalled:
		t.Fatal("provider should not run until the queued follow-up acquires a worker slot")
	default:
	}
	...
	case <-time.After(5 * time.Second):
		t.Fatal("timed out waiting for queued follow-up to run after worker slot release")
	}
	...
	require.Eventually(t, func() bool {
		...
		return execs[0].Status == models.ExecCompleted
	}, 5*time.Second, 100*time.Millisecond)

	execs, err := h.execRepo.ListByTaskChronological(ctx, task.ID)
	require.NoError(t, err)
	require.Len(t, execs, 1)
	assert.Equal(t, models.ExecCompleted, execs[0].Status)
	assert.Equal(t, "queued followup completed", execs[0].Output)
...
func TestHandler_Chat_ReconnectPreservesProjectID(t *testing.T) {
	...
	assert.Contains(t, body, `data-project-id="`+project2.ID+`"`,
		"chat root must have the requested project ID")
	assert.Contains(t, body, `h-full flex flex-col min-w-0 max-w-full`,
	...
	assert.Contains(t, body, `id="chat-messages" class="flex-1 min-h-0 overflow-y-auto pt-4 pb-1`,
	...
	assert.NotContains(t, body, `-mr-[29px]`,
		"chat messages pane must not use fixed right-margin scrollbar compensation because it leaves bubbles visually shorter than the input")
