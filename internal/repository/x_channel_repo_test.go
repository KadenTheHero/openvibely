...
func TestXChannelRepositoriesProjectIsolationAndReceiptLease(t *testing.T) {
	...
	claimed, err := receipts.Claim(ctx, "tweet-1", p1.ID, now, time.Minute)
	require.NoError(t, err)
	require.True(t, claimed)
	claimed, err = receipts.Claim(ctx, "tweet-1", p1.ID, now.Add(30*time.Second), time.Minute)
	require.NoError(t, err)
	require.False(t, claimed)
	claimed, err = receipts.Claim(ctx, "tweet-1", p1.ID, now.Add(2*time.Minute), time.Minute)
	require.NoError(t, err)
	require.True(t, claimed)
	require.NoError(t, receipts.Complete(ctx, "tweet-1", ""))
	claimed, err = receipts.Claim(ctx, "tweet-1", p1.ID, now.Add(4*time.Minute), time.Minute)
	require.NoError(t, err)
	require.False(t, claimed)
}
