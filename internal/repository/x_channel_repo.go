...
func (r *XInboundReceiptRepo) Release(ctx context.Context, tweetID string) error {
	_, err := execBoundSQLite(ctx, r.db, `DELETE FROM x_inbound_receipts WHERE tweet_id=? AND status='processing'`, tweetID)
	return err
}
