...
func customAutomationProducerScope(candidate models.AutomationDraftCandidate, actionRole string) string {
	...
	for _, edge := range candidate.Edges {
		...
		if !sourceOK || !targetOK || target.Role != actionRole ||
	...
	if len(names) == 0 {
		return "the connected producer stages"
	}
