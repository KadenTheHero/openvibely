...
- GitHub inbox: type trigger, role github_inbox. GitHub inbox is itself the scheduled Task and uses the same Schedule config fields. Its prompt must perform the complete assigned-issue mailbox job: only process assigned issues created by connected upstream producers in the same Automation, identify those producer stages by name and purpose in the inbox prompt, skip unrelated repository issues even when assigned, inspect and reconcile existing work, create or continue one issue-linked implementation Task per eligible approved issue, and start eligible work safely. Never add a separate Schedule before it.
