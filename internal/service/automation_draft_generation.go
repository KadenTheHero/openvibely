...
- Approved inbox: type trigger, role native_inbox. It is itself the scheduled Task and uses the same Schedule config fields. Its prompt must perform the complete approved-notification mailbox job: list approved notifications for the persisted project, inspect and claim them, create and link implementation Tasks, and complete, fail, or release processing safely.
...
- GitHub mailbox family: Schedule or Task -> Create GitHub issue -> Human assignment -> GitHub inbox, with a separate substantive Schedule -> GitHub inbox source, then GitHub inbox -> Task -> Open pull request -> Human review -> Outcome.
...
- GitHub inbox needs exactly one Schedule source and exactly one Human assignment source. It needs exactly one Task target, and that Task must use category active, have exactly one incoming edge from the inbox, and have exactly one outgoing edge to Open pull request. The Schedule source must describe concrete work of its own; never use a relay-only prompt such as "Initiate processing of approved issues", "Start the inbox", or "Trigger inbox processing". A valid Schedule prompt names the actual scheduled inspection or preparation it performs, such as "Review the assigned issue queue, identify newly assigned actionable issues, and record the issue numbers for downstream processing."
