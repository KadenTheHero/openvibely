...
- Approved inbox: type trigger, role native_inbox. It is itself the scheduled Task and uses the same Schedule config fields. Its prompt must perform the complete approved-notification mailbox job. Only process approved notifications created by connected upstream producers on that inbox's own approval branch in the same Automation, identify those producer stages by name and purpose in the inbox prompt, and state that runtime eligibility uses exact trusted Automation/graph/branch provenance rather than content similarity or model-supplied metadata. Call list_alerts without project_id, using decision_state=approved, implementation_task_linked=false, and bounded stable pagination. Do not pass the read filter, so both read and unread approvals are eligible. Inspect and claim results; create and link implementation Tasks; start each linked Task; and complete, fail, or release processing safely. The runtime automatically uses this scheduled Task's persisted project. Never search for or reuse a project ID from prior messages, examples, memory, tool output, the project snapshot, or the user description.
...
- GitHub inbox: type trigger, role github_inbox. GitHub inbox is itself the scheduled Task and uses the same Schedule config fields. Its prompt must perform the complete assigned-issue mailbox job: only process assigned issues created by connected upstream producers on that inbox's own assignment branch in the same Automation, identify those producer stages by name and purpose in the inbox prompt, state that runtime eligibility uses exact trusted local Automation/graph/producer-branch creation records rather than labels or issue-content similarity, inspect and reconcile existing work, create or continue one issue-linked implementation Task per eligible approved issue, and start eligible work safely. Never add a separate Schedule before it.
