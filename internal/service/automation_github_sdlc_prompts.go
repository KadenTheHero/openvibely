...
const githubSDLCDevInboxTaskLookupGuidance = ` For each GitHub issue, perform one existing-task lookup with ` + "`" + `list_tasks` + "`" + ` using the issue number or URL as the ` + "`" + `query` + "`" + `. Treat that single lookup result as the reconciliation result for that issue. Do not retry the same issue lookup, do not vary task lifecycle filters to search again, and do not run title or fragment searches for the same issue.`
