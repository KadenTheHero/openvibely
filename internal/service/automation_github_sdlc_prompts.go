...
const githubSDLCDevInboxBroadListTasksGuidance = ` For GitHub issue reconciliation, omit ` + "`" + `category` + "`" + ` and ` + "`" + `status` + "`" + ` for a broad search across all visible task categories and statuses. Supplying ` + "`" + `category` + "`" + ` or ` + "`" + `status` + "`" + ` restricts results to only that lifecycle state and can exclude otherwise matching tasks; do not add them when checking whether any matching task exists. Do not enumerate lifecycle state combinations after an empty result with ` + "`" + `total=0` + "`" + ` and ` + "`" + `has_more=false` + "`" + ` for that issue query; use a different query such as the exact proposed task title only when issue-number/URL matching is inconclusive.`
