...
const githubSDLCDevInboxTaskLookupGuidance = ` For each GitHub issue, perform one existing-task lookup with ` + "`" + `list_tasks` + "`" + ` using the issue number or URL as the ` + "`" + `query` + "`" + `. Treat that single lookup result as the reconciliation result for that issue. Do not retry the same issue lookup, and do not vary task lifecycle filters to search again. Use a different query such as the exact proposed task title only when issue-number/URL matching is inconclusive.`
