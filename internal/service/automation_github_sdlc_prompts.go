...
const githubSDLCDevInboxBroadListTasksGuidance = ` For GitHub issue reconciliation, omit ` + "`" + `category` + "`" + ` and ` + "`" + `status` + "`" + ` for a broad search across all visible task categories and statuses. Supplying ` + "`" + `category` + "`" + ` or ` + "`" + `status` + "`" + ` restricts results to only that lifecycle state and can exclude otherwise matching tasks; do not add them when checking whether any matching task exists. A returned ` + "`" + `filter` + "`" + ` object only echoes the parameters sent to ` + "`" + `list_tasks` + "`" + `; if it contains ` + "`" + `category` + "`" + ` or ` + "`" + `status` + "`" + `, do not treat those values as runtime defaults. If a call accidentally included ` + "`" + `category` + "`" + ` or ` + "`" + `status` + "`" + `, count that lookup as the reconciliation attempt for that issue and do not retry the same issue query just to obtain an unfiltered echo. Do not enumerate lifecycle state combinations after an empty result with ` + "`" + `total=0` + "`" + ` and ` + "`" + `has_more=false` + "`" + ` for that issue query; use a different query such as the exact proposed task title only when issue-number/URL matching is inconclusive.`
