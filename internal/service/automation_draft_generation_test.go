...
func TestAutomationDescriptionPromptMatchesStrictCustomValidationContract(t *testing.T) {
	...
	_, err = svc.PreviewDescription(context.Background(), "Review a request", models.AutomationCapabilitySnapshot{}, func(_ context.Context, prompt string) (string, error) {
		...
		require.Contains(t, prompt, "exact trusted Automation/graph/branch provenance rather than content similarity or model-supplied metadata")
		...
		require.Contains(t, prompt, "exact trusted local Automation/graph/producer-branch creation records rather than labels or issue-content similarity")
