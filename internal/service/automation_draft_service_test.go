...
func TestGitHubSDLCPromptsUseRepositoryFallbackAndTrustedLocalDeduplication(t *testing.T) {
	...
	require.Contains(t, githubSDLCDevInboxPrompt, "count that lookup as the reconciliation attempt for that issue")
	require.Contains(t, githubSDLCDevInboxPrompt, "do not retry the same issue query just to obtain an unfiltered echo")
	...
	require.Contains(t, prompt, "Use the listed issue details directly when they include the issue number, URL, title, body, labels, assignees, and state needed for task creation")
	require.Contains(t, prompt, "do not call github_get_issue for every listed issue as a default step")
...
func TestCustomAutomationValidatesGitHubHandoffsAndRejectsHumanBoundaryBypasses(t *testing.T) {
	...
	require.Contains(t, prompt, "count that lookup as the reconciliation attempt for that issue")
	require.Contains(t, prompt, "do not retry the same issue query just to obtain an unfiltered echo")
