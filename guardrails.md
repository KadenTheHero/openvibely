...
- Keep task-card delete `X` button resting-state styling theme-parity aligned. Do not add a light-only default circular fill (for example `.card .btn-circle.btn-ghost { background-color: ... }`) that diverges from dark mode; reserve emphasis for hover/focus/error states.
...
- Chat/thread markdown rendering must not pass raw model text directly into `marked.parse` when it may contain HTML/template fragments. Escape raw HTML-like tags first (while preserving fenced/inline code) so malformed model output cannot break bubble DOM/layout.
...
- **Sequential fast-forward merges must refresh stale task branches** — `git merge --ff-only` fails after prior merges when the task branch is behind target branch even without true content conflicts. Before FF merge, auto-rebase task branch onto current target branch when worktree path is available; if rebase conflicts, run `git rebase --abort`, keep `merge_status=conflict`, and return actionable update/rebase conflict messaging.
