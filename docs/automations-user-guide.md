...
Open a saved Automation from `/automations` to see its full-width `Live` graph. Current waiting, running, blocked, failed, and completed state appears on the graph itself. Live uses the same node size, typography, and graph scale as the Edit canvas so the two views remain visually consistent. The graph card fits the available desktop page height, and a one-node Automation keeps a normal node scale rather than stretching the node to fill the canvas.
