...
Open a saved Automation from `/automations` to see its full-width `Live` graph. Current waiting, running, blocked, failed, and completed state appears on the graph itself. Live uses the same node geometry, typography, coordinates, and padded graph bounds as the Edit canvas, so matching graphs retain the same visual node scale, including one-node and small graphs. The graph card fits the available desktop page height.
