...
| `DISABLE_UPDATE_NOTIFICATIONS` | Hides idle update offers and prevents packaged download/staging/apply while retaining release checks | `1,true,yes,on,0,false,no,off` | `true` for packaged standalone binary and desktop builds; `false` for source, Hosted, and Docker |
...
Packaged standalone binary and desktop builds always perform the startup/daily signed release check so the service can collect update metrics, but update offers are hidden until signing is provisioned. Update checks include a random 128-bit pseudonymous install identifier that is stable across days and upgrades, rotates every 90 days, and is stored only in the app-data update state rather than the application database. The hosted service stores only an HMAC of this identifier and never joins it to a user account or session. Set `OPENVIBELY_DISABLE_INSTALL_ID` (to any value, including empty) to omit the identifier and avoid generating or storing one. With the default `DISABLE_UPDATE_NOTIFICATIONS=true`, OpenVibely does not expose an idle update notification and does not download, stage, or install the returned release. Set `DISABLE_UPDATE_NOTIFICATIONS=false` to enable packaged update offers. Source checkout metric-only behavior and externally controlled Hosted/Docker behavior keep their existing defaults. An update accepted before notifications were disabled remains visible and completes recovery so changing the setting cannot abandon a drained installation.
