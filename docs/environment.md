...
| `CLAUDE_CODE_MAX_OUTPUT_TOKENS` | Overrides the per-call output token budget for Claude Code (claude-code adapter) API calls. When set to a positive integer, the value is applied up to the model's hard upper limit. | Positive integer string | Model-specific defaults: `64 000` for claude-opus-4 latest, `32 000` for claude-sonnet-4/claude-opus-4-0/claude-opus-4-1, `8 192` for claude-3-5-sonnet/haiku | unset \| `32000` |
