...
- Add/connect/delete/move canvas interactions remain draft-only. An invalid Save stays in the editor with setup errors and creates no task or Scheduler resources; partial compiler failures preserve the existing publication-attempt resource report for safe retry.
...
- Editing a published Automation still clones an immutable draft version. Direct Save publishes that new version without mutating prior version history, existing invocations, or human approval/review/merge/release/deploy boundaries.
...
- `TestAutomationBlankBuilderIsEmptyInteractiveAndPersistsNodeActions` additionally failed first when a settings-panel Delete connection submit inherited the Save intent and attempted publication. It now proves node/edge mutation submits stay draft-only with no redirect or resources.
...
- [x] Add Automations-page `New Automation` with Template, Describe It, and Blank paths only, draft graph/editor, assumptions/warnings/errors, failure preservation, and success navigation to Live. The later Phase 5 direct-save contract removed the original web preview/confirmation controls and routes.
