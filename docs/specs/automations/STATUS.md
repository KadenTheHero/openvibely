...
- Automation-owned prompt coverage for Offering Manager, all three finders, Dev Inbox, and Loop Auditor, plus cadence coverage and a source-level regression that forbids runtime dependencies on `builtinskills`, `SKILL.md`, or global skill support files. Adapter contract revision `14` invalidates pre-fix private confirmations rather than allowing generic or global-only prompts to be saved later.
