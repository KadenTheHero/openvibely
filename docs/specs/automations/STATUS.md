...
- The selected authoritative `automation_graphs.md` view records checkpoint `08aa489` and its prior exact-version web guard, so this latest direct product correction still requires authoritative reconciliation through the after-complete memory path; no direct managed-memory mutation tool is exposed in this implementation turn.
