...
- A valid new Save stages and immediately publishes one immutable version. A valid edit Save requires the exact non-empty published `base_version_id`, stages one successor, atomically updates the Automation name/description/type with the active-version switch, supersedes the prior immutable version, and leaves no version in `draft` state. Missing or stale base versions return the submitted graph with a reopen message and no persistence or resource effects; non-Save edit mutations remain operable without the token.
- Planner validation discards its short-lived staged version. Compiler failure may retain unpublished version/journal rows only as internal retry/reconciliation evidence; they are not editable web product state. Chat retains its separate persisted candidate, displayed plan, and later explicit confirmation contract.
...
- `TestAutomationWebBuilderKeepsUnsavedChangesBrowserLocal` failed first because opening Blank inserted an Automation and version. The stale-save follow-up then failed first because omitting `base_version_id` returned `204` and published a successor. It now proves transient new/edit mutations without a required token, missing/mismatched Save-token rejection with no history changes or staged draft, invalid-Save no-persistence, refresh/re-entry restoration from the published graph, valid immediate publication, immutable successor history, and zero editable draft versions.
...
- Existing public Save, custom topology, standalone Schedule, Describe It/Chat parity, project-isolation, and production Chrome Edit-operability regressions pass under the corrected persistence boundary.
...
- Automation-focused handler/service/repository tests and the production Automation page/Chrome suite pass.
- `TMPDIR=/private/tmp go test ./internal/... -count=1 -timeout 180s` and a fresh `TMPDIR=/private/tmp go test ./... -count=1 -timeout 180s` pass every package after the stale-save hardening.
- The selected authoritative `automation_graphs.md` view correctly describes browser-local edits and immediate Save, but its progress text still says the now-fixed missing-base guard is pending. This task runtime exposes only read-only `memory_view` and no authorized managed-memory mutation action; the tracked copy was deliberately not edited independently. Authoritative reconciliation remains required when a scoped writer is available.
