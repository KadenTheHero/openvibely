...
Phases 1-4 and the prior graph/editor, publication, runtime, identity, safety, pagination, accessibility, and observability repairs remain complete through `d2512dd`. Phase 5 now has a validated custom foundation, deterministic multi-task execution, Native human approval, configurable GitHub lifecycle capabilities, Describe It/Chat parity for the surfaced custom schema, repaired Blank connector and schedule resource ownership, and a direct-save web publication flow. Blank starts with `adapter_key=custom`, the node dialog uses one `Node purpose` field with no runtime/design-only split, no preset nodes or transitions are inserted, and its empty supported-edge palette is serialized as `[]` rather than `null`, so direct node-to-node drags cannot fail in the browser before creating an edge. Visible lines and arrows render fully opaque in a final pointer-transparent foreground above nodes and connector circles, while selection hit areas remain behind nodes and captured pointer releases resolve the real connector under the release coordinates. Saving from the Automations-page builder validates and immediately publishes the complete browser-local candidate through the existing planner/compiler, with no persisted editable web draft and no separate Review and apply or Apply changes controls/routes; canvas mutations remain browser-local, invalid Save returns the submitted candidate with no definition or resource effects, and Chat retains its later explicit confirmation contract. Applying a custom Schedule → Agent task graph creates a dedicated scheduled task and Scheduler row for the Schedule node plus a separate ordinary Backlog Agent Task. A due occurrence dispatches the Schedule node's own task through the existing Automation machinery; successful completion follows the exact immutable Schedule-to-Agent edge through the existing task-chain path and activates the Agent Task without repointing the Scheduler row. The Schedules calendar therefore shows every applied Schedule node as its own scheduled task using the immutable schedule-node name. Unsupported capability handoffs remain fail-closed setup errors, and publication derives only real task/schedule effects plus explicit Alert/GitHub/human-gate configuration from user-owned nodes and edges. Runtime Alert and GitHub activity uses existing services and exact immutable Automation-version provenance. Per the user's correction, the old JSON-only hidden Workflow subsystem is not an Automation builder capability and will be removed separately rather than exposed here. The audit findings at `42646bb` were repaired: Chat-persisted candidates have no editable web route or portfolio card, and compiler failures resume the exact staged Automation/version/plan journal rather than staging duplicates. A later direct product correction removed `base_version_id` from the web Save contract: Save now replaces the active publication under the same Automation identity without a user-facing version token, while immutable internal versions remain for invocation/history provenance. Removing or reconfiguring a Schedule node now produces an explicit deterministic `delete` publication effect and atomically deletes the superseded Scheduler row instead of leaving a disabled card on the Schedules page. Describe It generation failures replace the HTMX page with a visible error and retained description instead of returning an unswapped `422`, while successful generation explicitly identifies the graph as browser-local until `Save changes`. Its exact edge schema is repeated to the independent repair call, and generated numeric Schedule/Agent Task priorities are now bounded to the supported `1..4` task scale without weakening strict validation for browser-authored Save candidates. Remaining Phase 5 work is authoritative managed-memory reconciliation for the latest Describe It correction, followed by the separately required fresh edit-free full-objective audit.

## Phase 5 Breadcrumb And Edge Deletion Follow-up

### Completed Requirements

- Automation pages now follow the app's established breadcrumb hierarchy instead of rendering standalone back buttons. Live, History, and Definition use `Automations / <Automation name>`; new/edit and deeper invocation/work-item pages extend the same hierarchy with their current page where needed.
- Breadcrumb links retain project context and use the shared HTMX navigation path, preserving browser Back/Forward history and server-authored page titles.
- The redundant canvas-toolbar `Delete connection` button and its duplicate JavaScript state/listener are removed. Selecting a line exposes its topmost midpoint `×` delete control and reconnect endpoints; keyboard Delete/Backspace behavior remains available.

### Regression And Validation Evidence

- `TestAutomationGraphThemeAndHistoryNavigationInChrome` failed first because Automation detail pages had no breadcrumb. It now traverses Live, History, Definition, Blank, and edit flows through the rendered breadcrumb links, requires the toolbar delete button to be absent, clicks the selected edge's real midpoint `×`, and verifies the browser-local candidate removes the edge.
- `TestAutomationBlankBuilderIsEmptyInteractiveAndKeepsNodeActionsTransient` requires the duplicate toolbar marker to be absent while retaining the complete transient builder and Save contract assertions.
- Stable `templ generate`, `git diff --check`, `go build ./...`, `go test ./web/templates/pages -count=1 -timeout 180s`, `TMPDIR=/private/tmp go test ./internal/... -count=1 -timeout 180s`, and `TMPDIR=/private/tmp go test ./... -count=1 -timeout 180s` pass. Desktop build/test emits only the documented non-failing newer-SDK linker warnings.

## Phase 5 Describe It Priority Normalization Follow-up

### Completed Requirements

- The Describe It prompt now defines the existing task priority contract explicitly: integer `1..4`, where `1` is low, `2` normal, `3` high, and `4` urgent. Because the complete original prompt is included in the bounded repair request, both independent model calls receive the same rule.
- Numeric priorities produced outside that range for generated Schedule and Agent Task nodes are clamped to the nearest supported value before capability validation. This correction is limited to model-generated candidates; browser-authored Save candidates still receive the canonical validation error and cannot persist an invalid priority.
- Malformed priority types remain validation failures and can use the one bounded repair attempt. The correction does not relax strict JSON decoding, unknown-field rejection, topology validation, capability validation, or project isolation.

### Regression And Validation Evidence

- `TestAutomationDescriptionGenerationNormalizesOutOfRangeTaskPriority` failed first because the generation prompt omitted the range and the first invalid numeric value consumed repair. It now proves generated Agent Task priority `5` becomes urgent priority `4`, generated Schedule priority `0` becomes low priority `1`, no repair call is consumed, and the same manually submitted candidate remains invalid through the normal Save validator.
- The existing expanded custom-builder, unsupported-schema repair, exact edge-schema repair, and no-persistence generation regressions pass.
- `templ generate` is stable with zero generated updates; `gofmt`, `git diff --check`, and `go build ./...` pass. Desktop build/test emits only the documented non-failing newer-SDK linker warnings.
- `TMPDIR=/private/tmp go test ./internal/... -count=1 -timeout 180s` and `TMPDIR=/private/tmp go test ./... -count=1 -timeout 180s` pass every package.

## Phase 5 Describe It Failure Feedback Follow-up

### Completed Requirements

- Describe It sends the default model a strict graph schema, the surfaced custom node and handoff contract, the user's description, and a bounded server-derived snapshot of the selected project's agents, agent tool names, integrations, supported roles, and safety boundaries. The call receives no mutation tools and cannot create an Automation or resources itself.
- A successful generation returns a browser-local graph. The builder now explicitly tells the user to review it and select `Save changes`; only that single Save validates and creates the Automation and its real resources.
- A provider, parse, repair, or capability-validation failure now returns an HTMX-swappable `200` fragment with a visible `Could not generate a supported Automation` alert and the submitted description retained. Ordinary non-HTMX form submission retains semantic `422` behavior.
- Requests that require unsupported executable semantics, such as arbitrary market-data result conditions, remain fail closed. Describe It does not invent unknown nodes, arbitrary code, or unsupported conditional handoffs.

### Regression And Validation Evidence

- `TestAutomationDescribeFailureIsVisibleAndPreservesInput` failed first on the hidden `422`. It now exercises a stock increase/decrease candidate with an unsupported price-result condition, proves one bounded model repair attempt, visible retained failure feedback, and zero Automation/task/Scheduler effects.
- `TestAutomationCanonicalChatRuntimeExecutesPreviewDraftPlanAndConfirmedPublish` now verifies the analogous existing HTMX failure path returns the visible fragment while preserving Chat's separate preview, persisted plan, and later-confirmation behavior.
- `TestAutomationGraphThemeAndHistoryNavigationInChrome` submits the visible production Describe It form in Chrome, observes the error replacement, and verifies the stock-monitor description remains editable before continuing through the existing Blank/edit/Save graph interactions.
- `templ generate`, `gofmt`, `git diff --check`, `go build ./cmd/server`, the complete Automation-focused repository/service/handler/production-Chrome suite, `TMPDIR=/private/tmp go test ./internal/... -count=1 -timeout 180s`, and `TMPDIR=/private/tmp go test ./... -count=1 -timeout 180s` pass. The full suite emits only the known non-failing macOS newer-SDK linker warnings.

## Phase 5 Browser-Local Edit Follow-up

### Completed Requirements

- Opening Template, Describe It, Blank, or `Edit automation` no longer creates or clones an Automation/version. The rendered candidate and every add/connect/delete/move/settings mutation stay in browser-carried `candidate_json`; refresh or navigation reconstructs the published graph and discards unsaved edits.
- `Save changes` submits the complete candidate once. Save-time validation runs before persistence; invalid new or edited graphs return to the builder with setup errors and leave Automation/version/task/Scheduler counts unchanged.
- A valid new Save stages and immediately publishes one immutable version. A valid edit Save requires no web version token: it replaces the active publication under the same Automation identity, atomically updates the Automation name/description/type with the active-version switch, supersedes the prior immutable internal version, and leaves no version in `draft` state. Immutable versions remain an internal runtime/history provenance mechanism rather than a user-facing Save workflow.
- Planner validation discards its short-lived staged version. Compiler failure retains unpublished version/journal rows only as internal retry/reconciliation evidence and returns their scoped Automation/version/plan identity with the exact candidate. Retrying Save requires that unchanged candidate, unchanged plan revision, project scope, and an existing failed compiler attempt, then resumes the same journal and reconciles existing resources without staging another Automation or version. Chat retains its separate persisted candidate, displayed plan, and later explicit confirmation contract; fabricated web retry fields cannot create an attempt for an unconfirmed Chat draft.
- Chat-persisted candidates are excluded from the web portfolio and published detail/definition views. The former version-addressed draft GET/POST routes and persistence-on-mutation handler are removed, while Chat's internal draft, planning, confirmation, and publication services remain available to the canonical Chat actions.
- Unsaved new builders no longer expose Delete because no Automation exists. Persisted Automations retain confirmed Delete and project isolation.

### Regression Evidence

- `TestAutomationWebBuilderKeepsUnsavedChangesBrowserLocal` first reproduced persisted web drafts, then the obsolete missing-token guard. It now proves transient new/edit mutations, invalid-Save no-persistence, refresh/re-entry restoration from the published graph, edit Save without `base_version_id`, active-publication replacement under the same Automation identity, immutable internal history, and zero editable draft versions.
- `TestCustomAutomationReplacementDeletesRemovedSchedule` failed first because a removed Schedule node produced no delete plan effect and publication left its Scheduler row disabled. It now proves a deterministic `delete` effect and complete Scheduler-row removal; the replacement failure/retry regression also proves reconfigured schedules retain prior behavior until the atomic switch, then remove the superseded row.
- `TestAutomationWebSaveRetriesExactFailedPublicationJournal` uses a real SQLite schedule-insert failure after the scheduled task is created. It proves the response carries exact retry identity and that retry completes the same Automation, version, publication attempt, task, and schedule with no duplicates.
- `TestAutomationChatDraftIsNotExposedAsEditableWebState` proves a Chat-created candidate has no portfolio card, published detail, or version-addressed editable route; attempted web mutations leave it unchanged, its Chat planner remains available, and fabricated retry fields cannot publish it without an existing confirmed compiler attempt.
- `TestAutomationBlankBuilderIsEmptyInteractiveAndKeepsNodeActionsTransient` carries the candidate through real public mutation responses and proves node/edge changes create no Automation, task, or Scheduler rows.
- The production Chrome regression enters Blank through `New Automation`, edits only a published Automation, and retains immediate editor operability plus browser-local graph interaction and single-Save coverage.

### Validation Status

- `templ generate` is stable with zero updates; `gofmt`, `git diff --check`, and `go build ./...` pass with only the documented non-failing macOS newer-SDK linker warnings.
- The complete Automation-focused repository/service/handler and production Chrome suites pass, including web Save replacement without a version token, explicit removed-Schedule deletion planning, atomic Scheduler cleanup, Chat confirmation, persisted-candidate isolation, and exact failed-publication retry.
- `TMPDIR=/private/tmp go test ./internal/... -count=1 -timeout 180s` and `TMPDIR=/private/tmp go test ./... -count=1 -timeout 180s` pass every package after the replacement-Save and Schedule cleanup corrections.
- The selected authoritative `automation_graphs.md` view records checkpoint `18a6a16`, including replacement Save and Schedule cleanup. It still requires reconciliation with the later Describe It failure-feedback repair through the after-complete memory path because no direct managed-memory mutation tool is exposed in this implementation turn.
- Remaining work is authoritative managed-memory reconciliation and the separately required fresh edit-free full-objective audit from the clean correction checkpoint.

## Phase 5 Direct Save Follow-up

### Completed Requirements

- The Automations-page builder has one publication action: `Save changes`. It saves the latest candidate, computes the deterministic publication plan, invokes the existing idempotent compiler when validation succeeds, and redirects to the Automation's Live view.
- Add/connect/delete/move canvas interactions remain browser-local. An invalid Save stays in the editor with setup errors and creates no Automation, version, task, or Scheduler resources; partial compiler failures preserve the internal publication-attempt resource report for safe retry.
- The separate `Review and apply` and `Apply changes` controls, review panel, confirmation token fields, and web plan/publish routes are removed. Chat retains its separate displayed-plan and later-user-confirmation security contract.
- Editing a published Automation reconstructs its immutable active version in browser memory without cloning a version. Direct Save creates and publishes one new immutable version without mutating prior version history, existing invocations, or human approval/review/merge/release/deploy boundaries.

### Regression Evidence

- `TestAutomationBlankAppliedStandaloneScheduleUsesScheduleNodeNameOnSchedulePage` now fails first without direct Save, then proves canvas editing creates no resources and one Save creates the Schedule node's real task and Scheduler row, redirects to Live, and exposes the entry on the Schedules page.
- `TestAutomationBuilderWebSavePublishesImmediatelyAndIsProjectScoped` proves one visible-form Save publishes configured resources through the shared planner/compiler and preserves project isolation.
- `TestAutomationBuilderSavesUnsupportedCustomConnectionsWithoutExecutingThem` proves an unsupported graph remains editable after Save and creates no partial tasks or schedules.
- `TestAutomationBlankBuilderIsEmptyInteractiveAndKeepsNodeActionsTransient` proves settings-panel Delete connection remains a browser-local mutation with no redirect, persisted Automation/version, or runtime resources.
- The production Chrome graph regression clicks the actual top `Save changes` button, proves its submitted `FormData` carries `save_changes=true` plus the latest graph state, and rejects any second Review/Apply control.

### Validation Status

- `templ generate`, formatting, `git diff --check`, and `go build ./...` pass. Only the documented non-failing macOS newer-SDK linker warnings appear.
- The Automation-focused repository/service/handler/Chat/observability suite and `go test ./web/templates/pages -count=1 -timeout 180s` pass, including the production Chrome Save and graph interaction regression.
- `TMPDIR=/private/tmp go test ./internal/... -count=1 -timeout 180s` passes every internal package. The first final `go test ./...` run had the known unrelated Task Thread Chrome scroll-position timing failure; that exact test passed immediately in isolation, and a fresh `TMPDIR=/private/tmp go test ./... -count=1 -timeout 180s` rerun passed every package.
- Remaining work is the separately required fresh edit-free full-objective audit.

## Phase 5 Edit Navigation Follow-up

### Completed Requirements

- `Edit automation` now submits through an explicit connected HTMX navigation source rather than depending on stale element processing after HTMX history restoration. The POST still carries the real form values, targets `#main-content`, and honors the server-authoritative canonical draft `HX-Push-Url`; the ordinary form action remains the no-JavaScript fallback.
- The live Automation root is fully replaced by one builder root, so the editor retains normal page placement and its canvas, Add node control, node-purpose dialog, and other options are immediately operable without a manual refresh.

### Regression And Validation Evidence

- The production Chrome regression now returns to a Live Automation through HTMX history, clicks the visible `Edit automation` POST control, and proves the builder mounts without a residual Live root and opens the visible Add node dialog. Before the repair the request fell through to native navigation and the browser runner disappeared at that exact click.
- `templ generate`, `gofmt`, `git diff --check`, and `go build ./...` pass. Desktop build/test emits only the documented non-failing newer-SDK linker warnings.
- The focused Automation database/repository/service/handler/Chat suite and `go test ./web/templates/pages -count=1 -timeout 180s` pass, including the production Automation, Chat navigation, and Task Thread navigation Chrome fixtures.
- `TMPDIR=/private/tmp go test ./internal/... -count=1 -timeout 180s` and `TMPDIR=/private/tmp go test ./... -count=1 -timeout 180s` pass every package.
...
- The compiler/adapter contract version is `8`; publication-plan goldens were deliberately refreshed so stale version-7 confirmations cannot apply the Schedule deletion semantics.
...
- Authoritative managed memory now reflects the composable capability checkpoint `35761a3`, including task fan-out and the completed Schedule/task ownership and edge-control repairs. A separate strictly read-only full-objective audit remains pending after the final clean checkpoint.
...
- `TestAutomationBlankAppliedScheduleUsesScheduleNodeNameOnSchedulePage` failed first after a complete visible Blank create/connect/Save flow: the real Scheduler row was returned, but `/schedule` titled it with the target Agent task. It now proves the saved schedule is rendered as `Weekday review` and not `Review support queue`.
- `templ generate`, `gofmt`, `git diff --check`, and `go build ./...` pass; desktop emits only the documented non-failing newer-SDK linker warnings.
- `go test ./web/templates/pages -count=1 -timeout 120s` passes, including the production Chrome graph fixture.
- `go test ./internal/... -count=1 -timeout 120s` passes every internal package.
- `TMPDIR=/private/tmp go test ./... -count=1 -timeout 120s` passes every package.
...
- Maintained adapters retain their existing schedule-to-task behavior. Publication plans and compiler revisions include the additional custom Schedule task effect, and changed legacy-shaped custom schedules are replaced rather than silently reused.
...
- `TestAutomationBlankAppliedScheduleUsesScheduleNodeNameOnSchedulePage` now proves the full visible Blank create/connect/Save path persists distinct tasks, links the Scheduler row to the Schedule task, leaves the Agent Task in Backlog, and renders the Schedule node on `/schedule`.
...
- Direct managed-memory mutation was unavailable at this checkpoint. The authoritative topic was later reconciled with composable task fan-out and optional approval-result behavior; the current repair still requires its own authoritative update as recorded in the active status above.
...
- [x] Preserve draft-only geometry, project isolation, explicit Automation identity, direct web Save, Chat confirmation, idempotency, trigger ownership, lifecycle, and no-inference boundaries.
...
- Direct Save creates only the configured producer/inbox tasks and their schedules. GitHub issue, assignment, implementation-template, PR, and review effects remain planner/compiler configuration and create no external object during publication. Issues, implementation tasks, and PRs arise only through the existing GitHub runtime, task creation, task-PR, queue, and worker services.
...
- Direct Save preserves the planner distinction between visible task/schedule resource effects and notification-handoff/human-gate configuration. Publication creates no Alert; the compiled Agent task uses the existing `create_notification` runtime capability, and the real pending project Alert is created only when that task runs.
...
- [x] Normalize Template, Describe It, Blank, fixed Chat candidate, and published-version reconstruction inputs through one deterministic draft service with server-generated IDs/layout and shared validation.
...
- [x] Persist mutable unpublished versions for Chat confirmation while keeping published topology immutable; reconstruct active versions into browser-local web edit state and retain all historical invocation/work-item version references.
- [x] Add constrained schema-driven node/edge editing and validation with explicit affected-node and summary errors. Canvas mutations and draft generation create no task, schedule, Alert, issue, execution, goal, Workflow, or PR; the later Phase 5 direct-save contract makes the visible web Save the explicit publication action.
...
- [x] Add Automations-page `New Automation` with Template, Describe It, and Blank paths only, a browser-local graph/editor, assumptions/warnings/errors, failure preservation, and success navigation to Live. The later Phase 5 direct-save contract removed persisted editable web drafts plus the original web preview/confirmation controls and routes.
...
- Added failed replacement evidence proving the prior published version and trigger remain active, then verified successful retry creates one new cadence-specific trigger, deletes the superseded Scheduler row, and reuses the task.
...
- [x] Replace visible `Edit as new draft`, draft badge, and Save draft terminology with Edit automation and Save changes. The later direct-save follow-up removed the temporary Review and apply / Apply changes controls.
- [x] Reconstruct the active immutable definition into browser memory for repeated edits without creating or overwriting a working draft version; Save creates one immutable successor only after validation succeeds.
...
- [x] Remove the redundant always-present Disconnect selected toolbar action; selecting a line exposes its midpoint `×` delete control while Delete/Backspace keyboard support remains available.
...
- The same Chrome fixture selects a painted connection, exposes and clicks its midpoint `×`, removes the edge without replacing the builder, and confirms the edge is absent from `candidate_json`; endpoint-drag rewiring remains covered in the same mounted graph.
...
- [x] Use the selected line's midpoint `×` as the direct canvas delete action instead of a redundant `Delete connection` toolbar button; retain selected-edge endpoint rewiring and Delete/Backspace behavior.
...
- [x] Suppress the selected edge group’s browser bounding-box outline while preserving selected-line emphasis, draggable endpoints, the red midpoint delete control, and keyboard deletion; the later redundant toolbar delete action is removed.
