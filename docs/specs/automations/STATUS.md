...
- The maintained GitHub SDLC template and bootstrap skill share one bundled canonical prompt file. Both paths use the same Offering Manager, finder, Dev Inbox, and Loop Auditor behavior plus daily finder, hourly inbox, and weekly auditor cadences. Registration hydrates the exact bound Task/Schedule configuration into the graph and replaces older `{}` graph configuration without mutating the bound Task prompt.
...
- Exact canonical GitHub SDLC prompt and cadence parity between the bootstrap skill and Automation template for Offering Manager, all three finders, Dev Inbox, and Loop Auditor. The adapter contract revision invalidates pre-fix private confirmations rather than allowing generic prompts to be saved later.
