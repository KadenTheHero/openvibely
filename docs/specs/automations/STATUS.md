...
- A valid new Save stages and immediately publishes one immutable version. A valid edit Save requires the exact non-empty published `base_version_id`, stages one successor, atomically updates the Automation name/description/type with the active-version switch, supersedes the prior immutable version, and leaves no version in `draft` state. Missing or stale base versions return the submitted graph with its original empty/stale token and a reopen message, so repeated submission cannot adopt the current version and overwrite newer work; normal edit mutations retain the token from the editor that submitted them.
...
- `TestAutomationWebBuilderKeepsUnsavedChangesBrowserLocal` failed first because opening Blank inserted an Automation and version. The first stale-save follow-up failed because omitting `base_version_id` returned `204` and published a successor; the audit follow-up then failed because rejection returned the current token beside the stale candidate. It now proves transient new/edit mutations, missing/mismatched token rejection, empty/stale token retention across responses, repeated stale resubmission rejection, invalid-Save no-persistence, refresh/re-entry restoration from the published graph, valid immediate publication, immutable successor history, and zero editable draft versions.
...
- The complete Automation-focused repository/service/handler and production Chrome suites pass, including Chat confirmation, persisted-candidate isolation, repeated stale submission rejection, and exact failed-publication retry.
- `TMPDIR=/private/tmp go test ./internal/... -count=1 -timeout 180s` and `TMPDIR=/private/tmp go test ./... -count=1 -timeout 180s` pass every package after all three audit repairs.
- The selected authoritative `automation_graphs.md` view accurately records the `42646bb` audit findings but has not yet been updated with these repairs. This task runtime still exposes only read-only `memory_view` and no authorized managed-memory mutation action; the tracked copy is deliberately not edited independently. Authoritative reconciliation remains required before the strict audit.
- Remaining work is managed-memory reconciliation and the separately required fresh edit-free full-objective audit from the clean repair checkpoint.
