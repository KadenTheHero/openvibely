...
- A valid new Save stages and immediately publishes one immutable version. A valid edit Save stages one successor, atomically updates the Automation name/description/type with the active-version switch, supersedes the prior immutable version, and leaves no version in `draft` state.
...
- `TestAutomationWebBuilderKeepsUnsavedChangesBrowserLocal` failed first because opening Blank inserted an Automation and version. It now proves transient new/edit mutations, invalid-Save no-persistence, refresh/re-entry restoration from the published graph, valid immediate publication, immutable successor history, and zero editable draft versions.
...
- `TMPDIR=/private/tmp go test ./internal/... -count=1 -timeout 180s` and a fresh `TMPDIR=/private/tmp go test ./... -count=1 -timeout 180s` pass every package.
