...
- Focused regressions pass under the race detector for exact-execution GitHub issue task creation, zero persistence on invalid provenance or missing explicit `repo_url`, hostile repository-override isolation, atomic work-item binding, sequential and concurrent canonical Task reuse, task-bound channel routing, Automation-owned GitHub SDLC prompts, post-activation Pause/Archive admission, maintained schedule intent across Pause/Resume, concurrent cross-run GitHub issue deduplication, fail-closed issue/PR topology and causal-task provenance, explicit URL-only external/PR resolution, and idle web plus queued/channel plus failed-retry Task Thread preparation.
...
- `TMPDIR=/private/tmp go test -p 1 ./internal/repository ./internal/service ./internal/handler ./web/templates/pages -count=1 -timeout 300s` passes all affected packages.
