...
- The maintained GitHub SDLC Automation reads the Offering Manager, finder, Dev Inbox, and Loop Auditor prompt patterns directly from the bootstrap skill shipped with OpenVibely, while retaining daily finder, hourly inbox, and weekly auditor cadences. The bootstrap skill package is not modified. Registration hydrates the exact bound Task/Schedule configuration into the graph and replaces older `{}` graph configuration without mutating the bound Task prompt.
...
- Exact shipped-skill prompt coverage for Offering Manager, all three finders, Dev Inbox, and Loop Auditor, plus Automation cadence coverage. The parity regression reads the repository's built-in `SKILL.md` independently and compares every generated/persisted Task prompt, while the existing bootstrap skill package and its catalog coverage remain unchanged. Adapter contract revision `14` invalidates pre-fix private confirmations rather than allowing newer global-only or generic prompts to be saved later.
...
The implementation and tracked documentation now describe GitHub SDLC Automation defaults sourced directly from the bootstrap skill shipped with OpenVibely, matching recurring cadences, and registration hydration; the bootstrap skill package remains unchanged. The selected authoritative `automation_graphs` managed-memory view still records the pre-fix prompt drift, while `.openvibely/memories/automation_graphs.md` predates this shipped-skill parity checkpoint. The current runtime exposes `memory_view` but no authorized scoped memory writer, so those managed representations cannot be reconciled in this turn and synchronization is not claimed. The separate public documentation repository still needs an Automation Graphs page and navigation links, but its checkout contains unrelated uncommitted skill changes and was left untouched.
