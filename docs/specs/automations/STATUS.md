...
- Automation GitHub runtime paths require the current project to have an explicit `repo_url`; they never infer a repository from a local Git remote and ignore model-supplied repository overrides across reads and writes. Automation-bound Task Thread follow-ups route GitHub tools through the same assignment gates, activity idempotency, provenance recording, and duplicate policy as initial and scheduled runs. Issue creation performs a bounded one-page server-side normalized-title duplicate check under a short durable project/repository/title-fingerprint lease, so concurrent runs cannot both create the same issue; the lease stores no issue content, the check returns no issue content to the model, and rejection exposes only the issue number plus a runtime-constructed same-repository URL.
...
- No pre-commit root execution, production provenance lookup, stale queued-root refresh after graph replacement, completed root Live projection, paused/archived queued-root demotion, paused existing/new root admission on Resume, existing-root post-commit submission, lifecycle-gated downstream handoffs with exact paused Resume recovery, and deterministic task handoffs/fan-out.
...
- Native Alert and GitHub lifecycle boundaries, explicit project `repo_url` enforcement with no local Git-remote inference, project isolation for the full Automation GitHub tool surface, hardened Automation-bound Task Thread follow-up routing, same-execution issue mutation idempotency, and bounded concurrency-safe server-side cross-run issue duplicate prevention without model-visible issue content or persisted issue text.
...
The implementation, tracked documentation, and selected managed `automation_graphs` memory describe the same self-contained GitHub SDLC Automation prompt defaults, recurring cadences, registration hydration, browser-local creation paths, and atomic Save contract. The Automation has no runtime dependency on the bootstrap skill package.
