...
## Task Prompt Rewrite: Execution Root vs. Main Checkout

**The inbox/assigning LLM cannot know the worktree path in advance.** Worktree assignment happens at task-creation time, after the inbox model has already composed the task prompt. Any `## Repository /main/checkout/path` block written by the inbox model is therefore always the wrong path for the executing agent.

**The fix is entirely server-side** — do not rely on prompt instructions to the inbox model to self-correct:

1. Worktree assignment and task creation are co-located in the same path. At that point `task.WorktreePath` is already known.
2. The task-creation path must rewrite the stored `tasks.prompt` before saving/running:
   - **Remove** any `## Repository <main-checkout-path>` block generated by the inbox model.
   - **Inject** a canonical `## Execution Root <task.WorktreePath>` block, with a note that this is the repository root for this task run.
   - If `project.RepoPath` must appear at all, label it clearly as `Project source checkout: `/main/checkout`` and note it must not be used as the task working root.
3. Do not leave both blocks present. Server owns execution-root text; the LLM does not.
4. `project.RepoPath` is legitimate project metadata (useful for admin/system agents, `DisableRuntimeWorktree` agents, project info UI, debugging). The bug is specifically that it leaked into **implementation-task prompt generation as if it were the working root**. Fix the exposure at source, not by instruction.

**Project-info tool fix (5-point spec):**
1. Stop returning `Repository: /main/checkout/path` to worktree-managed agents via the project-info tool.
2. For worktree-managed agents, return `Repository path: hidden for worktree-managed agents` (or omit the field).
3. Expose only the execution root to normal task agents: `Repository root: <task.WorktreePath>`.
4. `create_alert_implementation_task` (and equivalent task-creation paths) must reject or server-rewrite prompts containing `project.RepoPath` as the operative repository.
5. Add a targeted test: given project repo path `/repo/main` and task worktree `/repo/main/.worktrees/task_x`, an implementation task prompt must not contain `/repo/main` as `## Repository`; it must contain `/repo/main/.worktrees/task_x` as `## Execution Root`.

**Source of the `## Repository` block:** the block is model-generated by the Native SDLC inbox agent writing `project.RepoPath` obtained from project-info tool results (`channelProjectInfoResult` in `chat_action_runtime.go`, `executeProjectInfo` in `chat_processing.go`). No runtime layer generates or hardcodes that heading. Fix the source (project-info tool output) and the storage path (task-creation prompt rewrite), not the inbox model prompt.

**Complement to the system-prompt/cwd fix (completed 2026-08-23):** the system-prompt worktree-orientation fix is necessary but insufficient — if `tasks.prompt` itself names the main checkout as `## Repository`, the model is explicitly told the wrong root in its task body, overriding the system-prompt worktree sentence. Both fixes are required together.
...
- Treat conflict recovery as repository mutation too, not as a UI-only action. Resolve and Abort paths must acquire the same process-wide canonical Git-common-directory lease used by merge and rebase; menu-wide `hx-disabled-elt` and pre-lease handler checks are not synchronization. Hold the lease across the complete AI resolution operation, including its read/modify/stage/commit work, and re-fetch the task plus verify that the active merge/conflict belongs to that task while the lease is held. Abort must perform the same lease-held ownership/state check before aborting or restoring paths, so a resolver, another task, or a new merge/rebase cannot mutate the repository concurrently.
- Every action endpoint must return a response compatible with the initiating HTMX target and caller surface. Card-originated merge, rebase, and PR requests target `#kanban-board`, so a lease-time eligibility change must return a non-2xx card error or a board-compatible authoritative board fragment; it must never return a successful `200` Changes-tab fragment that HTMX can swap over the Kanban board. Changes-tab callers may refresh `GetTaskChanges`, but audit each source-specific error branch and test stale-in-lease Rebase separately from Merge.
- For card-originated merge, rebase, and PR actions, classify the caller before early lookup or validation failures. A missing/deleted task and every stale or otherwise ineligible card request must return a non-2xx response, or a board-compatible authoritative board fragment when appropriate; do not return `204` with only a failure toast because the shared card modal treats every `2xx` response as success, closes, and can leave stale board state.
- Project-scoped card requests must not turn that response fix into an existence oracle: missing/deleted task IDs and existing-but-foreign task IDs should use the same caller-facing not-found contract, normally `404`, while retaining failure feedback and no board replacement fragment. Preserve any intentionally different non-card legacy behavior separately, and pair the missing and foreign cases in regression tests.
...
- If an audit finds that merge/rebase are serialized but Resolve or Abort still mutates the same repository without that lease, treat it as a material correctness gap even when the browser disables the other menu buttons. Trace every conflict-recovery mutation path, require lease-held ownership revalidation, and add concurrent recovery-versus-recovery and recovery-versus-merge/rebase regressions before declaring the audit clean.
...
- **For task-prompt rewrite**, add a targeted test: given `project.RepoPath = /repo/main` and `task.WorktreePath = /repo/main/.worktrees/task_x`, the stored `tasks.prompt` must not contain `/repo/main` as `## Repository`; it must contain `/repo/main/.worktrees/task_x` as `## Execution Root`. Also add a test that the project-info tool does not return `Repository: /repo/main` when called from a worktree-managed task agent context.
- Add concurrency regressions proving Resolve and Abort share the canonical lease with Merge and Rebase across separate service instances and repository aliases. Hold a lease through the complete AI-resolution window, verify another recovery or merge/rebase is rejected, and verify ownership/state is rechecked before Abort or Resolve mutates files or commits.
- For every card-originated action that can target `#kanban-board`, add a lease-held stale-eligibility regression and assert the error response is non-2xx or otherwise board-compatible. In particular, cover stale-in-lease Rebase separately from Merge; a successful Changes-tab fragment is not a valid response for a board swap and can replace the Kanban board with the wrong surface.
- For card-originated action races and deletion, assert the caller-specific HTTP status and HTMX behavior, not only the toast text or server log. Cover the task disappearing before the initial lookup and after any lease-held validation; a card `Create PR`/merge/rebase failure must not use `204` merely because the response carries a failure toast, since the shared modal interprets all `2xx` responses as success and may close without refreshing the stale board.
- Pair missing-task and existing-but-foreign card requests for every project-scoped action and assert the same caller-facing not-found contract, normally `404`, so a non-2xx repair does not become a task-existence oracle. Keep separate assertions for non-card callers when their legacy response contract is intentionally different.
