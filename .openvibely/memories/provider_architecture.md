...
- The 2026-06-28 Mixture of Models reference fan-out race audit was fixed by replacing shared `results` slice progress scans with an atomic completed-reference counter; ordered reference results remain for post-join aggregation/context only.
- The 2026-06-28 Mixture of Models slot-callability audit was fixed by centralizing eligibility in `LLMConfig.IsCallableMixtureSlot`: mixture slots reject recursive, duplicate, missing, CLI-backed OpenAI/Anthropic, hidden/internal/unknown, and other non-callable rows. The Models UI picker uses the same callable non-mixture eligibility and feeds browser-side mixture option data through a safe `data-options` attribute.
- The 2026-06-28 Mixture of Models runtime-tool isolation audit was fixed by adding `llmcontracts.WithoutRuntimeTools(ctx)` and using it for mixture reference requests before applying the per-reference timeout. Reference contexts now mask inherited runtime tools while preserving cancellation/deadline behavior; aggregator requests keep the original runtime-tool context.
- The 2026-06-28 Mixture of Models reference-ordering audit was fixed by replacing indirect multi-select ordering with a direct ordered selected-reference list in the Models dialog. Users add callable references from an available-model picker, select one ordered reference, and move it up/down relative to other selected references; create/edit hydration and submitted `mixture_config_json` preserve that configured order. A hidden comma-ordered `mixture_reference_ids` fallback is kept synchronized for native form fallback.
- The 2026-06-28 Mixture of Models reference prompt-wrapping audit was fixed by adding `AgentRequest.RawDirectPrompt` and setting it on mixture reference subrequests. Anthropic API and OpenAI-compatible direct adapters honor this raw advisory mode by skipping normal OpenVibely task headers/system prompts; Anthropic raw reference calls also suppress provider-native web tools. Aggregator requests keep normal provider behavior.
...
- The Models UI treats Mixture of Models as a no-credential virtual provider with aggregator/reference pickers from existing callable non-mixture configs, a direct ordered reference list, an enabled toggle, numeric controls, edit hydration, and cost-warning copy.
