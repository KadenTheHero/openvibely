...
- Mixture slot eligibility is centralized in `LLMConfig.IsCallableMixtureSlot`: mixture slots reject recursive, duplicate, missing, CLI-backed OpenAI/Anthropic, hidden/internal/unknown, and other non-callable rows. The Models UI picker uses the same callable non-mixture eligibility and passes browser-side mixture option data through a safe `data-options` attribute.
