...
source_id: 8837543b3d212564719100cb96dda7ad:be11869b8795f0ee
confidence: high
...
- Known review-submission project scoping gap `#779`: `POST /tasks/:taskId/reviews/submit` loads by raw task ID and can submit comments or start follow-up work for a task outside the selected/requesting project without a project-ownership check.
...
- Repeated 2026-08-20 PR handoff incidents showed the same failure pattern: stale/polluted remote PR heads despite clean local task candidates. Durable response is to verify local candidate scope, live source/PR heads, live PR file list, target branch, and validation before claiming publication is current.
- Current actionable publication states retained from those incidents: PR `#719` for issue `#699` still needs authorized branch replacement/update and fresh audit; PRs `#721`, `#746`, and `#781` were manually repaired but still need fresh separate audit-only review before completion. PR `#781` for issue `#771` was realigned on 2026-08-21 by resetting the local task branch to live PR/source head `9c98b8daa87921f13a43192d6f0c8f9e7464c978` after preserving polluted local tip `26cc260e` at `backup/issue-771-local-before-handoff-cleanup-20260821194006`; validation passed before handoff.
...
- A local merge error saying the worktree is not on the expected task branch indicates branch/metadata drift. Diagnose the exact assigned worktree path with status/ref evidence and identify which lineage branch owns the diff.
...
- If the browser does not show expected merge actions while the server-rendered Task Changes HTML does, suspect stale HTMX/page state or dropdown visibility before lineage/ancestry.
