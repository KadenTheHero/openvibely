...
- Goal continuation must enqueue normal task-thread follow-ups through `thread_inputs`; it should not start work inline or rely on deterministic auto-enqueue code after every successful follow-up. The Goal Agent should evaluate goals as a real lifecycle/system agent with its granted runtime tools.
