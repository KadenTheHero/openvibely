...
- Do not assume a dedicated `create_task` runtime tool is exposed in every chat/task-thread turn. Task creation may be triggered by `[CREATE_TASK]` marker compatibility where enabled, by runtime action surfaces when available, or by local app HTTP/database APIs in coding-agent contexts; choose the surface that matches the current runtime and avoid claiming a callable `create_task` tool exists unless it is actually listed.
