...
- Known bug tracked in GitHub issue #10: attachment copying is independent per created task, but activation is gated by one global `allReady` flag. If one destination copy fails, sibling tasks whose attachments copied successfully are also stranded in Backlog instead of being activated independently.
