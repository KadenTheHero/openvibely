...
- Chat orchestration should support both explicit and implicit task-goal creation. Explicit examples may use a `Goal:` field; implicit examples include completion-condition phrasing such as “keep going until a final audit-only review confirms there are no remaining material bugs or regressions,” which should be inferred as the task goal rather than only copied into the prompt.
- Goal continuation must enqueue normal task-thread follow-ups through `thread_inputs`; it should not start work inline or rely on deterministic auto-enqueue code after every successful follow-up. The Goal Agent should evaluate goals as a real generic `after_complete` lifecycle/system agent with its granted runtime tools for any task turn that has an active/evaluable goal, including ordinary worker task completions and task-thread follow-ups.
- The user wants goal status mutation tools such as `mark_task_goal_achieved` and `report_task_goal_blocked` available to any explicitly granted agent, not protected-Goal-Agent-only; this includes task-thread assigned agents and lifecycle-hook agents. Ungranted/default agents should not see or execute those tools, and execution must retain stale `goal_id` plus current `status='active'` guards so paused/cleared/replaced goals reject stale evaluator writes. Lifecycle hook status-tool execution should authorize against the actual lifecycle hook agent's grants when hook-agent context is present; protected Goal Agent authority comes from `system_kind=goal` hook identity, not caller-supplied runtime-origin fields.
...
- Task-goal prompt context should be present across task-follow-up entry points, including web direct sends, queued task-thread sends, review submissions, and channel-origin task runs. Review and channel-origin runs should reactivate an achieved goal before prompt construction, resolve the effective assigned agent definition, and append the same grant-aware `taskGoalContext` used by web task-thread follow-ups so the model sees the persisted goal objective/status and accurate goal-tool guidance.
...
- `ResumeGoal` only resumes paused goals, preserves the same `goal_id`, and clears blocked audit state. Stale guarded goal-status updates should return a stale-update error rather than reporting success with a null goal.
- Goal Agent status writes such as achieved/blocked must require the same `goal_id` and current `status='active'`; paused or cleared goals should stale-reject in-flight evaluator writes so lifecycle evaluation cannot revive or complete a no-longer-evaluable goal.
- Kanban task cards render a `Goal` badge from a derived `Task.HasGoal` flag populated by the board/list task query for non-cleared goals; other task-loading paths currently leave `HasGoal=false`, so future task card/link surfaces need their queries expanded if they rely on the flag.
- Goal Agent generic `after_complete` evaluation should reload the latest persisted goal after the lifecycle hook before publishing `TaskGoalEvaluated`; otherwise a post-hook event can report stale pre-evaluation achieved/blocked state after `TaskGoalService` has already published the updated event.
- Current behavior: when a user/manual task-thread follow-up starts on a task whose goal is `achieved`, OpenVibely reactivates that same goal to `active` before prompt context and lifecycle evaluation so generic after-complete Goal Agent evaluation rechecks it. Goal Agent/system-agent continuations are excluded from this reactivation path so internal follow-ups do not reopen completed goals.
- Slack/Telegram task-goal runtime tools are unavailable stubs; web/API/task-thread goal tooling is implemented.
