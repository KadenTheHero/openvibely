...
- Stale `running` task executions whose owning task is terminal/inactive or has no real active worker must be repaired or ignored before active-turn decisions. Completed/cancelled task-thread follow-ups should start a new follow-up execution instead of queueing behind such stale rows; startup recovery should terminalize impossible stale runs; manual Start/move-to-active should promote the oldest pending task-thread queued input rather than silently rerunning the original prompt.
