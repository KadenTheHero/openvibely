...
source_id: memory_consolidation_2026_08_21
confidence: high
...
- Open bug `#767`: browser Chat's `Default` model selector resolves through the global default before loading the current `project_id`, so it ignores `projects.default_agent_config_id`. Fix should resolve the selected model after project context is loaded and add browser Chat regression coverage for a project-specific default differing from the global default.
...
- Known Chat inspection/control-plane gaps: no prompt-safe detail view for a specific agent's capabilities/configuration (`#595`), no Chat action to create/save reusable user-managed Agent profiles from the conversation (`#732`), no Chat action to update existing non-protected user-managed Agent safe fields after creation (`#755`), no Chat action to create/update custom personality prompts already managed in the browser (`#777`), no read-only Chat action to inspect a task's linked GitHub PR record/state (`#765`), no project skill catalog discovery action (`#621`), no live worker-capacity pressure action (`#604`), and no Insights query action (`#624`).
...
- Resolved `#725`: browser Task Detail edit validates `agent_definition_id` with the shared primary-Agent resolver before saving; blank submissions clear assignment, and cross-project, unknown, disabled, archived, or non-selectable Agents return controlled `400` without persistence. PR `#744` added browser edit regression coverage for valid project-scoped assignment, valid global selectable Agent assignment, and blank clearing.
