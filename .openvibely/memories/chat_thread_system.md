...
source_id: e5522988d7d63fc218b4f92ae6ca8b95
confidence: high
...
- As of task `f076cd4c16ee53c0a0e05418c388f12f` on 2026-06-01, stale task-thread `running` execution recovery and manual queued-follow-up promotion safeguards were attempted, but repeated audit-only reviews still found ship-blocking gaps. Durable open concerns: runtime stale recovery can fail brand-new legitimate follow-up executions during the window before task status/category is updated; startup recovery can preserve stale `active/queued` task-thread runs after restart; manual queued-follow-up promotion may create parallel executions unless it verifies no real active execution exists; recovery requeues hidden steering rows but queued rows tied to stale executions also need safe promotion, cancellation, or retargeting. Future fixes should harden the entire lifecycle in one pass rather than patching one branch at a time.
