...
source_id: memory_consolidation_2026_05_28
confidence: high
...
Core agent and skill catalog decisions:
- The built-in system agent for skill work is Skill Curator, with clean `skill_curator` identifiers. Its scheduled maintenance skill is `maintain_skill_library`; bundled Skill Curator skill text should say “skill” when it means skill, not “agent.”
...
- Agents are global by default and reusable across projects. Project-scoped agents/skills live under `<project_root>/.openvibely/agents/...`; global agents such as Skill Curator and user-created global agents live under the global app/config agents root shared by local web/server and desktop modes unless explicitly overridden.
- The on-disk catalog is authoritative after seed: `<agents_root>/AGENTS.md` lists agents at that root, `<agents_root>/<agent_key>/SKILLS.md` lists that agent’s skills/metadata, and `<agents_root>/<agent_key>/skills/<skill_key>/SKILL.md` is the skill body.
...
- Assigned-agent tasks should use that agent’s merged agent-owned skill catalog, not the top-level standalone skill catalog. Runtime `skill_view` and available-skill rendering must inspect the same assigned-agent scoped catalog.
- Built-in/global agents and skills are synced through the embedded/built-in sync path. Bundled `SKILL.md` bodies may be overwritten as app-owned source; bundled `AGENTS.md`/`SKILLS.md` index files are only created when missing because the LLM/user owns them once on disk.
...
- Agent root `SKILLS.md` is the authoritative agent overview/index and metadata surface for the Agents page, lifecycle hooks, normal task loading, tool permissions, lifecycle hooks, and related declarations. It should link to actual skill files and be updated idempotently by preserving existing links and upserting new ones.
...
- Standalone skill declarations should stay compact and limited to current catalog/manager fields. Keep compact standalone selection metadata such as `routing.triggers`, `routing.priority`, and `routing.description` if it is active; do not reintroduce removed legacy agent-routing hint scaffolding.
- Generated agent or skill declarations must include the explicit frontmatter `kind` expected by the catalog/manager, such as `openvibely.agent_skill`, when required by current source.
...
- The hook should review the compacted backend LLM conversation snapshot used for the task, not persisted threads, diffs, summaries, execution artifacts, or separately invented truncation policies. Avoid passing bulky artifacts that can exceed provider input limits.
- Before saving learning, Skill Curator should inspect existing agents/skills as needed and avoid duplicate or already-covered learning. Absence of a suitable existing agent/skill is a reason to create durable coverage when the learning is reusable.
- Reusable cross-agent improvements belong in standalone skills. Assigned-agent skill or definition updates are reserved for behavior specific to that assigned agent’s role, purpose, private workflow, or selected agent-owned skill.
- Hook context/tool descriptions should label assigned agent identity, purpose, selected agent-owned skills, selected standalone skills, provenance, and write policy explicitly; do not rely on the model inferring scope from filesystem paths.
...
- Ordinary task agents that create or change skills may lack index-file access. Tool descriptions should tell them to update indexes only when they have access; otherwise they should mention changed skills in the completion summary for the after-complete hook to index.
...
- `task_mode` represents the primary active task execution slot/bookkeeping and should not be exposed as an ordinary user-authored lifecycle hook like `route_task`, `before_run`, or `after_complete` unless deliberately redesigned.
...
- In task detail UI, show the selected primary task agent from persisted assignment/effective mode. Lifecycle activity rows such as `route_task — skill_curator` identify hook executors, not necessarily the routed primary task agent.
...
- A lifecycle hook `OutputContract` constrains the final structured result stored/validated by lifecycle code, not the agent’s working notes, tool use, or reasoning during the session. If output validation fails with invalid JSON, inspect raw hook output and enforce JSON-only final output for the contract.
...
Agent editor and UI direction:
- The agent create/edit dialog should align with on-disk agent-owned skills, not older embedded generated-skill records. It should let users see/manage real `SKILL.md` files and `SKILLS.md` overview/index metadata.
- Label the agent-owned skill area simply as “Skills.” Avoid “Agent-Owned Skills,” legacy Routing-tab fields, and Model Defaults JSON editing in the Advanced tab for now.
...
- Lifecycle/skills work is incomplete unless wired end-to-end: build the skill catalog for each task turn, resolve hook skill bodies, run routing/effective-mode resolution before LLM execution, register the correct runtime tools, make created agents/skills visible in the filesystem catalog, execute scheduled bindings, and log enough to debug behavior.
