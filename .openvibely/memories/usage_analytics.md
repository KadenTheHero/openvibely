...
- Failed Task Patterns `LastFailedAt` values must be emitted as browser-parseable RFC3339/ISO UTC strings, e.g. `strftime('%Y-%m-%dT%H:%M:%SZ', MAX(e.started_at))`, before frontend `new Date(...).toLocaleString()` renders them; SQLite's default space-separated timestamp format can produce `Invalid Date` in browsers.
