...
- Chat/thread attachment sends should mark send intent at the actual form submit gesture (send button or Enter), then consume that intent after HTMX swaps/layout settling to bottom-align even when variable-size screenshots or other attachments change height. Render image attachments with a stable marker and bind shared image load/error smart-scroll correction on initial render and relevant swaps; image corrections should snapshot send intent before it is consumed and still stop if the user explicitly scrolls up after sending.
