...
- When an audit goal excludes managed memory or skills, make zero memory/skill tool calls for the entire audit turn, including orientation. Begin evidence gathering with repository tools. A violated audit is invalid and must be repeated in a fresh compliant turn.
