...
source_id: c15029778a997a3d9b569406da3cea51:21792faa2e14f92e
confidence: high
...
- When memory/skill reads are allowed but their drift must be ignored, strict audit turns must gather repository/worktree evidence before managed memory or skill reads. The audit verdict must be based on repository, worktree, PR, task-thread, and requirement evidence. Do not treat memory/skill drift, edits, stale facts, or changed skill instructions as audit findings unless the task explicitly asks to audit those artifacts.
- Redundancy Finder rotation state as of 2026-08-11: recent inspected/published duplication areas include task-goal runtime actions (`#333`), alert handler refresh tails (`#351`), Automation save validation (`#356`), task/worktree Changes diff-state resolution (`#378`), task-board kanban refresh assembly (`#383`), dashboard page-shell rendering (`#392`), execution stream terminal mapping (`#440`), attachment orphaned-file cleanup (`#458`), agent create/update dialog payload parsing (`#471`), browser/runtime schedule mutations (`#479`), project create/update repository-source handling (`#484`), and model/provider configuration form-to-model normalization (`#500`). Future Redundancy Finder runs should pick a different bounded component unless explicitly asked to revisit those workflows.
