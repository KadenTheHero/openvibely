...
source_id: fa2066a725be105329aff6febe30b87a:84dff6801253b956
confidence: high
...
- GitHub Dev Inbox scans configured Authorized Users and PAT identity when available, deduplicates by repository/issue, and creates work after assignment approval and task/PR reconciliation. Assigned open issues are eligible even when manually created. For task reconciliation, prompt/tool/template guidance should use one broad `list_tasks` query per issue number/URL with `category` and `status` omitted; do not enumerate lifecycle-state filters after an exhausted empty result. `category`/`status` are ordinary `list_tasks` narrowing fields, not a separate GitHub-specific mode; keep generic tool semantics consistent. Resolved 2026-08-20: the shared `list_tasks` tool description plus GitHub Dev Inbox prompt/template/docs/bootstrap-skill wording explicitly say omitted `category`/`status` performs broad search while supplying either filter restricts results and can exclude matching tasks. This `list_tasks` reconciliation issue is GitHub SDLC-specific; Native Approved Inbox uses `list_alerts`, `claim_alert`, `create_alert_implementation_task`, `execute_tasks`, and alert-processing tools, so it does not need a Native maintained-template revision for this issue unless the Native prompt itself changes.
- 2026-08-20 Dev Inbox `list_tasks` investigations: persisted execution `c015ef9f69d8a067cdf5efb50ee3b822` had corrected broad-search prompt wording, but all 13 `list_tasks` result blocks echoed `category="active",status="pending"`; `ExecuteListTasksTool` only trims decoded fields and does not default omitted filters, so echoed `active/pending` meant the model supplied those filters. The overcorrection was reinforced by loaded project skill `openvibely_github_implementation_inbox_workflow`, whose fallback guidance treated echoed `active/pending` as omitted filters being unsupported. Later execution `8428a197b8e6990948520b4c856f5172` still used the latest prompt but tried local `gh`, a Python GitHub API fallback, and a post-creation `list_tasks` batch after creating tasks. Resolved 2026-08-20: GitHub Dev Inbox prompt/template/docs/bootstrap-skill wording now says inbox discovery must use the provided runtime GitHub tools only, not local shell commands, `gh`, Python scripts, `curl`, or direct GitHub API calls, and says a successful newly created Active `create_task` response is enough confirmation; do not call `list_tasks` again for that issue in the same inbox turn merely to verify start, labels, or existence. GitHub SDLC maintained-template revision is now 10 so lower-revision Automations can be offered an update.
- The runtime duplicate guard for `list_tasks` preserves generic tool semantics while caching exhausted-empty input keys across intervening read-only tools during an execution; duplicate exhausted-empty calls return a non-hidden `duplicate_noop` response. The cache clears after write actions such as `create_task`, so later discovery can see newly created or changed tasks.
...
- Resolved 2026-08-20 audit finding: `docs/github-autonomous-sdlc-user-guide.md` no longer tells users to post GitHub issue-status comments from the Dev Inbox, aligning the guide with the maintained GitHub SDLC Dev Inbox prompt/template prohibition; copied custom prompts created before the fix can still retain the forbidden behavior.
- Resolved 2026-08-20 issue `#705`: Custom Automation child-task activation recompiles prompts from the saved published graph candidate/metadata so downstream handoff edges such as `open_pull_request` remain visible to prompt generation; the existing notification fallback behavior remains and activation must not materialize extra tasks, schedules, or graph resources.
- Resolved 2026-08-20 audit gap: custom/generated GitHub assignment handoff prompts emitted by `automation_save_validator.go` now carry the GitHub Dev Inbox fixes from maintained template/docs/bootstrap paths: use only provided GitHub runtime tools for inbox discovery, use broad `list_tasks` reconciliation with omitted `category`/`status`, and do not call `list_tasks` again for the same issue after a successful newly created Active `create_task` response. Regression coverage asserts these constraints for the custom GitHub handoff prompt path.
