...
- GitHub Dev Inbox task reconciliation must use one broad `list_tasks` query per issue number/URL with `category` and `status` omitted. Supplying either field narrows ordinary generic `list_tasks` results and can exclude matching tasks; returned `filter` values only echo submitted parameters and are not runtime defaults or a reason to retry the same query; do not enumerate lifecycle states after an exhausted empty broad search.
...
- The 2026-08-20/2026-08-21 Dev Inbox fixes are GitHub SDLC-specific and carried by shared `list_tasks` tool wording plus GitHub Dev Inbox prompt/template/docs/bootstrap-skill wording. Native Approved Inbox uses alert-processing tools, so it does not need a Native maintained-template revision for this issue unless the Native prompt itself changes. GitHub SDLC maintained-template revision is `11` so lower-revision Automations can be offered an update.
