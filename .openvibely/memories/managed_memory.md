...
- The `before_run` `recall_memory` lifecycle output should expose compact selected-memory debug metadata, such as root-relative file/topic identifiers plus brief summaries/snippets, so the Lifecycle tab can show which memories were selected and injected without dumping raw unrelated memory content. Render selected memory identifiers compactly and deduped, route_task-style as inline badges/pills rather than repeated stacked list entries.
