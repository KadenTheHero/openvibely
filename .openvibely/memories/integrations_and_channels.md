...
- Channel Settings status badges use one shared configured/running/connected class/label contract across GitHub, Slack, Discord, Telegram, and Email, while provider adapters retain intentional wording differences. Configured-state precedence must win over a connected/stored installation when the integration is not actually configured.
...
- Dev Inbox duplicate prevention is existing-work-first: use one broad `list_tasks` lookup per issue number/URL, do not enumerate lifecycle filters or repeat the same lookup, and keep maintained prompts/templates aligned with this contract. Existing saved maintained Automations require explicit update/edit/save or recreation to adopt corrected prompt revisions.
...
- Open correctness defect `#842`: `github_list_assigned_issues_with_prs` exposes `limit`/`offset` pagination arguments but ignores both, returns the full PR-filtered result, and provides no continuation metadata. Callers cannot page this advertised action; regression coverage should exercise nonzero limits/offsets across the exposed runtimes.
...
- Assigned-issue list tools return compact paginated discovery records without issue bodies. After repository/issue deduplication, hydrate only likely actionable candidates through `github_get_issue`; do not detail-hydrate every assigned-list entry during scans.
