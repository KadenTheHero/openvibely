...
- Open duplication gap `#568`: inbound webhook create/update handlers duplicate Settings modal save-shape normalization for the same persisted fields (`name`, `enabled`, `system_instructions`, `default_priority`, prompt templates, and ordered `agent_ids`). Consolidate only the shared parse/normalize/apply assignment contract while keeping create-only secret generation and update-only ownership/load behavior separate; verify live GitHub/main state before treating this as merged or closed.
...
- Open email threading defect `#534` / duplicate latest Bug Finder issue `#563`: follow-up replies can fork into a new conversation when the mail client sets `In-Reply-To` to an outbound bot response `Message-ID` generated by the SMTP provider, because OpenVibely does not generate/persist outbound response message IDs and session lookup only recognizes the original user thread root/message ID. Regression coverage should prove replies referencing bot-response IDs rejoin the active email chat/history instead of starting a new email-origin task/thread.
