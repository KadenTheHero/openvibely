...
- Implementation for `#699`: Slack, Telegram, Discord, and Email channel runtimes wire `CreateGitHubProjectRuntimeOptions` into project handlers, so advertised `create_project` can create GitHub-backed projects and honor `switch_after_create` through channel active-project persistence.
- The read-only `list_channels` action is the Chat/control-plane path for compact channel readiness: it summarizes GitHub, Slack, Telegram, Discord, Email, inbound webhook counts, outbound target counts by platform/kind, and explicit-target policy using prompt-safe booleans/counts/status strings only. Web/API Chat uses the full handler summary; Slack, Discord, and Telegram channel runtimes wire service-side handlers so advertised tools are covered. Status paths use aggregate Slack/Discord/Telegram/Email auth counts plus grouped outbound-target summaries instead of materializing full allowlists/target rows; webhook status intentionally still uses endpoint-card listing because names/IDs are part of the prompt-safe summary.
...
- Resolved `#786`: Email and Slack inbound receipt `WithHandoff` repositories share atomic receipt recording, duplicate detection, and durable execution/queued-handoff persistence mechanics while preserving typed repo APIs, separate receipt tables, channel-specific identity keys, Email-only polling `Record`, Email no-`Message-ID` fallback behavior, and retry/duplicate-suppression semantics. A 2026-08-22 strict audit found no material issues; PR `#795` was open/non-draft against `main`, issue-scoped to four repository files, included `Closes #786`, and its live file blobs matched audited local `HEAD`.
...
- Resolved 2026-08-21 repeated-`list_tasks` incident: the fix clarifies filter echoes in the shared `list_tasks` tool description and exhausted-empty result note, maintained GitHub Dev Inbox prompt/template/docs/bootstrap skill, and custom/generated GitHub inbox handoff prompt. GitHub SDLC maintained-template revision was `11` for that fix; later body-free assigned-issue discovery updates raised the current revision to `15`. A strict read-only audit found no material issues after the repeated-lookup fix.
- Assigned-issue list entries are discovery records, not task-prompt hydration. After repository/issue deduplication, Dev Inbox should hydrate only likely task candidates with `github_get_issue` before creating work.
- Dev Inbox runs affected by assigned-list truncation or unintended `list_tasks` lifecycle filters should be rerun or reconciled against live GitHub/OpenVibely state before assuming assigned issues were handled. Existing workflow labels are not sufficient evidence of a usable implementation task mapping.
- Resolved 2026-08-22 Dev Inbox assigned-list truncation/detail-hydration incident: `github_list_my_assigned_issues` and `github_list_assigned_issues` remain compact paginated discovery indexes with every issue number/title/URL/state/labels/assignees/creator, but no `body_excerpt` or other body content. Dev Inbox should hydrate only selected actionable issues with `github_get_issue` before task creation. Internal assigned-list handling may filter and record Automation provenance, but must not call `postprocessAssignedIssues(...)`, `provider.GetIssue(...)`, or otherwise detail-hydrate entries during list scans. Regression coverage includes the 9-assigned-issue missed-middle shape (`#792`, `#791`, `#789`, `#783`, `#776`, `#768`, `#767`, `#766`, `#755`) and zero issue-detail requests for assigned-list scans; a 2026-08-22 fresh strict read-only audit found no material issues after the no-hydration fix.
- Current Dev Inbox reconciliation state from the 2026-08-22 assigned-issue run: issues `#792`, `#791`, `#789`, `#766`, and `#755` already had workflow labels but still need safe task-ID reconciliation because that run's single allowed `list_tasks` lookups accidentally carried lifecycle filters and returned no usable task IDs. Avoid treating labels alone as proof that OpenVibely has a usable implementation task mapping.
