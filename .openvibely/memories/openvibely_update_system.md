...
Drain lifecycle:
- For normal packaged installations, update checking is automatic but installation is user initiated through one Alerts action, `Update OpenVibely`. `POST /api/system/update/apply` is the single public local acceptance endpoint: it stages and verifies first, then automatically begins the drain and managed installation only after staging succeeds. The separate public staging route and button were removed; internal staging remains a distinct coordinator phase for durability and testing. Hosted installations remain the exception because an external administrator/control plane may direct updates according to hosted policy.
- The UI warns before acceptance that OpenVibely prepares the update and waits for active work to finish before restarting, so installation may not happen immediately. Acceptance and its exact drain lease are persisted before staging begins, and the acceptance endpoint returns once approval is durable. Accepted staging runs under a coordinator-owned lifecycle context, so browser/request cancellation cannot stop it. Restart recovery retries interrupted accepted staging, enters drain without restaging when the artifact was already persisted, and rebinds the exact drain generation if the drain was durably created before coordinator generation persistence. Explicitly retryable transport, timeout, recovery-pending, and persistence failures retain accepted intent and retry autonomously with bounded delay while admission stays open. Definitive metadata, artifact, configuration, permission, and unsupported-operation failures durably clear accepted intent, settle in `failed`, preserve the release for another user acceptance, and never begin drain. A failed definitive-settlement write retries settlement without rerunning installer staging. If `BeginDrain` succeeds but the `waiting_for_idle` coordinator write fails, the supervisor releases only that failed exact generation, preserves acceptance and the staged artifact, durably returns to `available`, and retries with a fresh drain generation without another click or restaging. Restart recovery likewise preserves approval when an ambiguous accepted `waiting_for_idle` record is paired with a durably idle drain. Periodic checks cannot replace an accepted release, and terminal settlement clears acceptance. `BeginDrain` closes the work tracker before observing active work, preventing new task, Chat, workflow, or automation executions from racing the idle snapshot.
- Production uses a 2-second quiet interval. It begins when tracked active work reaches zero, resets if work becomes active, and the coordinator polls drain status every 250 milliseconds, so readiness is normally observed about 2 to 2.25 seconds after continuous idle.
- Drain states are `idle` (admission open), `draining` (admission closed while tracked work finishes), and `ready` (active work remained at zero for the configured quiet interval). Existing work finishes normally; new ingress must remain queued or receive a retryable maintenance response.
- New application instances and rolled-back predecessors restore the persisted closed drain before workers start. Admission stays closed throughout successor health/version validation and rollback determination, and reopens only after success, rollback, or safe cancellation is durably settled.
- Each drain has a random exact generation. After final release revalidation, the installer takes durable ownership of that generation and records `applying` or `restarting`; stale operations cannot mutate or release a newer generation.
- An unowned drain may expire under its lease and is autonomously reset. Once replacement owns the generation, lease expiry cannot reopen admission underneath installation; explicit durable terminal reconciliation must release it.
- Admission reopens only after the reset is durably persisted, then a coalesced reopen signal resumes worker dispatch and scans durable queued Chat and task-thread work.
...
Latest local acceptance durability fix completed on 2026-08-02:
- Commit `5d25e56d` persists the user's accepted intent and exact drain lease before installer staging starts.
- Startup recovery retries interrupted accepted staging, advances an already-persisted staged artifact into drain without another user action or restaging, and binds the exact active drain generation after a crash between durable drain creation and coordinator generation persistence.
- Resumed staging failure retains retryable approval while admission remains open. Periodic checks cannot replace an accepted release, and terminal settlement clears the approval.
- Literal persisted-state regressions cover interruption during staging, staging completion before drain persistence, the drain/coordinator write gap, and resumed staging failure.

Latest accepted-staging autonomy and retryability fixes completed on 2026-08-02:
- Commit `0c135300` makes local acceptance return after approval is durably persisted and starts staging asynchronously under the coordinator lifecycle rather than the originating HTTP request context. Browser disconnect or request cancellation no longer cancels accepted staging.
- Commit `8cbe5df4` adds explicit `ErrUpdateRetryable` handling. Retryable transport, timeout, recovery-pending, transient HTTP/agent, and coordinator/drain persistence failures remain autonomously supervised; unknown errors are definitive by default.
- Definitive pre-drain failures durably clear accepted intent and settle in `failed` while admission remains open and the approved release remains available for a later user acceptance. If the settlement write fails, the supervisor retries only settlement and does not replay staging.
- Artifact transport failures, truncated downloads, HTTP 408/425/429/5xx responses, and Docker-agent responses marked retryable carry the retryable classification. Digest mismatch, unsupported artifacts/capabilities, invalid archives, configuration errors, permissions, and other unmarked failures settle definitively.
- Regression coverage proves request cancellation isolation, autonomous transient retry, definitive one-attempt settlement, later user retry, and retry of a failed definitive-settlement write without restaging.

Latest accepted drain-handoff persistence fix completed on 2026-08-02:
- Commit `e2d69359` adds acceptance-specific recovery when durable drain creation succeeds but persisting coordinator state as `waiting_for_idle` fails.
- The coordinator releases only the failed exact generation, preserves accepted intent and the staged artifact, durably restores the pre-drain `available` state, marks the failure retryable, and autonomously creates a fresh drain handoff without another user action or staging call.
- Reload normalizes accepted `waiting_for_idle` state back to `available` when the corresponding drain is already durably idle, covering an ambiguous coordinator write followed by successful drain cleanup before process exit.
- Regression coverage proves the first handoff write may fail transiently, the accepted update stages only once, the failed generation is released, a fresh generation reaches durable `waiting_for_idle`, and restart recovery retries without restaging.

Current audit status on 2026-08-02:
- A fresh separate strictly read-only audit of clean checkpoint `e2d6935953c70b2bcb0d0d16512731aa306dff36` found a material accepted-update retryability-classification defect; the workspace remained unchanged during the audit.
- `Client.Download` and `Client.Fetch` currently mark every `HTTPClient.Do` error retryable, including definitive redirect-policy failures, and mark a clean wrong-size artifact response retryable even though persistent mismatch against signed metadata is a definitive artifact-integrity failure. Accepted intent can therefore be retained and retried forever instead of settling for another user acceptance.
- `AgentHTTPClient.Do` currently returns response-body transport read failures without `ErrUpdateRetryable`, so a transient Docker-agent response interruption can be treated as definitive and clear approval.
- The accepted drain-handoff fix itself passed inspection: it preserves approval and staged artifacts, releases only the failed exact generation, retries with a fresh generation, and recovers the ambiguous persisted-transition/durable-idle restart boundary.
- The task remains incomplete. The retryability defects must be fixed in a separate implementation turn and followed by a fresh separate read-only audit from scratch.

Validation completed for checkpoint `e2d69359` on 2026-08-02:
- Focused handoff and restart regressions passed 20 race-enabled repetitions; the complete update package and update race suite passed.
- The native build, full uncached repository suite, Linux and Windows update-package compilation, scoped native/Linux/Windows vet, diff hygiene, and post-commit focused/build validation passed.
- The established non-failing macOS object-version linker warning remained.
