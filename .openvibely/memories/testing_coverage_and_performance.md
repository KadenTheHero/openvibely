...
- Large task-thread rendering uses worker-capable Markdown/code-range preparation, bounded marker-discovery slices, detached DOM construction, cooperative large-output insertion, serialized/cancellable completed hydration, render-revision deduplication, and polling that avoids remorphing unchanged completed transcripts. Current main still materializes complete tool OUT `<pre>` elements and skips task-link conversion inside tool output. Validation for lazy default-collapsed OUT rows, safe line-bounded tool-output links, expansion/collapse cost, and navigation caching belongs to `/Users/dubee/go/src/github.com/runbooks/openvibely-large-task-thread-rendering-performance-fix.md`; do not record its target measurements as current results until the implementation is on main and remeasured through the integrated Chrome page path.
